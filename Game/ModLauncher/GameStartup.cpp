		...
		case ESYSTEM_EVENT_CHANGE_FOCUS:
			{
				...
				if (!wparam && !gEnv->pSystem->IsDevMode() && !gEnv->pSystem->IsEditor() 
					&& pGame && pGame->GetIGameFramework() && pGame->GetIGameFramework()->IsGameStarted())
				{
					//todo: we miss the appropriate interface in IGame to do that
...
{  
  ...
  LOADING_TIME_PROFILE_SECTION(m_pFramework->GetISystem());
	...
	{
		MessageBox(0, "The correct usage is ModLauncher.exe -MOD NameOfTheMod", "Cannot launch", MB_ICONERROR);
		exit(0);
		return 0;
	}
}
...
//		filename.append("..\\");
		...
		IGame *mod = new ((void*)pGameBuffer) CGame();
#else
		IGame::TEntryFunction CreateGame = (IGame::TEntryFunction)CryGetProcAddress(m_modDll, "CreateGameMod");
		if (!CreateGame)
			return 0;
...
bool CGameStartup::InitWindow(SSystemInitParams &startupParams)
{
	...
	wc.hCursor       = LoadCursor((HINSTANCE)startupParams.hInstance, MAKEINTRESOURCE(102));
...
LRESULT CALLBACK CGameStartup::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SETCURSOR:
		{
			int iResource = IDC_CRYSISCURSOR_GREEN;
