	...
	if (pGameStartup->Init(startupParams))
	{
		...
		if ((pmec=pPM->RetrieveClientData(data))!=PMEC_NO_ERROR)
		{
			...
			case PMEC_KEY_INVALID:
				{
					ShowCursor(true);
					MessageBox( 0, "Could not find a valid License Key. Please run Sandbox to validate your CryENGINE installation.", 
						"Validation Error 0x900", MB_ICONINFORMATION | MB_OK | MB_APPLMODAL | MB_TOPMOST);
					CloseHandle(mutex);
					return 0;
				}
			}
			...
			{
				ShowCursor(true);
				ShowProtectionErrorMessage(0, pmec, &data);
		...
		if (isLevelRequested || isModRequested)
		{
			...
			CreateProcess("bin32/Launcher.exe", pBuffer, NULL, NULL, FALSE, /*flags*/ 0, NULL, NULL, &si, &pi);
		}
		else
		{
