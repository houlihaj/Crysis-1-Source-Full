...
bool CGame::Init(IGameFramework *pFramework)
{
  ...
  InitGameFactory(m_pFramework);
	...
	if(m_pDefaultAM)
	{
		...
		if(m_pDefaultAM->GetBindInfo(ActionId("screenshot"),actionMapBindInfo,iMaxKeys))
		{
			m_pDefaultAM->CreateAction("r_getscreenshot 2",eAAM_OnPress|eAAM_ConsoleCmd,actionMapBindInfo.keys[0]);
		}
		SAFE_DELETE_ARRAY(actionMapBindInfo.keys);
	}
