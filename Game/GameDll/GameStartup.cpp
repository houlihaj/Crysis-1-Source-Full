...
IGameRef CGameStartup::Init(SSystemInitParams &startupParams)
{
	...
	{
		strncpy(pModName, pModArg->GetValue(), 512);
		CryLog("Using Mod <%s> ...", pModName);
	}
	else
	{
		strncpy(pModName, GAME_NAME, 512);
	}
...
IGameRef CGameStartup::Reset(const char *pModName)
{
	...
	{
		MessageBox(0, "Unable to load Game or Mod DLL.", GAME_NAME, MB_ICONSTOP);
	}
...
void CGameStartup::RequestLoadMod(IConsoleCmdArgs* pCmdArgs)
{
	...
	filename.append("CrysisWars.exe");
...
bool CGameStartup::InitFramework(SSystemInitParams &startupParams)
{
	...
	// setup game specific constants inside cryengine

	ModuleInitISystem(m_pFramework->GetISystem());
	gEnv->pAISystem->SetConstant(IAISystem::eAI_CONST_bIsCrysis1, true);
	gEnv->pAISystem->SetConstant(IAISystem::eAI_CONST_fExplosionWarningRadius, 3.0f);
	gEnv->pAISystem->SetConstant(IAISystem::eAI_CONST_fPosAlongPathDistance, 10.0f);
