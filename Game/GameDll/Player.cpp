		...
		if(client)
		{
			...
			if(m_health < healthThrHigh && !g_pGameCVars->g_godMode)
			{
				...
				if(m_merciTimeStarted)
				{
					...
					{
						if (CNanoSuit* pNanoSuit = GetNanoSuit())
							health = lastHealth + pNanoSuit->GetHealthRegenRate() * frameTime;
					}
					lastHealth = health;
...
void CPlayer::UpdateSounds(float fFrameTime)
{
	...
	if(gEnv->bMultiplayer && gEnv->bServer && gEnv->bClient && !IsClient())
	{
		...
		{
			// make sure we have a voice listener. Bit hacky but I can't find
	...
	if (m_stats.downwardsImpactVelocity > 0.0f)
	{
		...
		if (velFraction > 0.0f)
		{
			...
			if (gEnv->bServer)
			{
				...
				// HACK PATCH2 : Apply previous downward velocity to make ragdoll fall with continuous velocity.
