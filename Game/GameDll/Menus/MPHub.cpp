...
void CMPHub::DisconnectError(EDisconnectionCause dc, bool connecting, const char* serverMsg/*=NULL*/)
{
	...
	case eDC_CDKeyChekFailed:
		{
			// was there a cd key already in registry, or did player enter it this time round?
			if(g_pGame->GetIGameFramework()->WasCDKeyReadSuccessfully())
			{
				// cd key was already there. Don't give the player the chance to edit it.
				ShowError(msg, true);
			}
			else
			{
