...
void CHUD::UpdateMissionObjectiveIcon(EntityId objective, int friendly, FlashOnScreenIcon iconType, bool forceNoOffset, Vec3 rotationTarget, bool characterhead, bool forceshow)
{
	...
	CryFixedStringT<256> name("");

	if(characterhead)
	{
		transX = vEntityScreenSpace.x*fScaleX+fHalfUselessSize;
		fSize = 0.7f + (0.5f * ((40.0f - min(40.0f,fDist)) / 40.0f));
		if(m_currentGameRules==EHUD_POWERSTRUGGLE)
		{
			name.append(m_pHUDTagNames->GetPlayerRank(objective));
			name.append(" ");
		}
	}
	name.append(pObjectiveEntity->GetName());
	SOnScreenIcon icon(objective, transX, transY, (int)iconType, friendly, fDist, fSize*fSize, -rotation, name.c_str());
...
/*	if(m_pHUDRadar->m_selectedTeamMates.size())
	...
	if(pActor && pGameRules)
	{
		...
		for(int m = 0; m < synchEntities.size(); ++m)
		{
			...
			if(vehicle)
				UpdateMissionObjectiveIcon(icon,friendly,eOS_TACTank, false, Vec3(0, 0, 0), false, true);
			else
			{
