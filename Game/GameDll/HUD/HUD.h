...
// uncomment following line to support G15 LCD
//#define USE_G15_LCD
...
public:
	...
	{
		SOnScreenIcon(EntityId _id, int _x, int _y, int _icontype, int _friendly, float _distance, float _size, float _rotation, const char* _text) : id(_id), x(_x), y(_y), icontype(_icontype), friendly(_friendly), distance(_distance), size(_size), rotation(_rotation), text(_text) {}
		EntityId id;
		int x;
		int y;
		int icontype;
