...
GVBool gviHardwareStartup(HWND hWnd)
{
	...
	GVIWaveFormat.nSamplesPerSec = GV_SAMPLES_PER_SECOND;
...
static GVBool gviPlaybackDeviceThink(GVIDevice * device)
{
	...
	// update the clock
	data->m_playbackClock += (GVFrameStamp)numFrames;
...
static void gviHardwarePlayPacket(GVIDevice * device, const GVByte * packet, int len, GVSource source, GVFrameStamp frameStamp)
{
	GVIHardwareData * data = (GVIHardwareData *)device->m_data;
	...
	//add it
	gviAddPacketToSourceList(data->m_playbackSources, packet, len, source, frameStamp, data->m_playbackClock);
}
...
static LPDIRECTPLAYVOICETEST GetVoiceTestInstance(void)
{
	...
	result = CoCreateInstance(&CLSID_DirectPlayVoiceTest, NULL, CLSCTX_INPROC_SERVER, &IID_IDirectPlayVoiceTest, &voiceTest);
