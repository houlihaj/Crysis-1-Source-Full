...
)
{
	...
	else if(type == MSG_SESSION_ID_TYPE)
	{
		...
		scSetReportIntention(gPlayerData.mStatsInterface, SCRACE_AUTHORITATIVE, &gPlayerData.mCertificate, &gPlayerData.mPrivateData,
...
// ********************************************************** //
			...
			else if(m_state == HOST_SEND_SESSID)
			{
				if(sessionCreated)
				{
					...
					scSetReportIntention(m_interface, SCRACE_AUTHORITATIVE, &gPlayerData.mCertificate, &gPlayerData.mPrivateData,
						setReportIntentionCallback, SCRACE_TIMEOUT_MS, NULL);
