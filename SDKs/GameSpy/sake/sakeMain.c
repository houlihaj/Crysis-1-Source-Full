...
gsi_bool SAKE_CALL sakeGetFileDownloadURL(SAKE sake, int fileId, gsi_char url[SAKE_MAX_URL_LENGTH])
{
	...
	rcode = _tsnprintf(url, SAKE_MAX_URL_LENGTH, _T("http://%S.sake.gamespy.com/SakeFileServer/download.aspx?fileid=%d&gameid=%d&pid=%d"),
		sake->mGameName, fileId, sake->mGameId, sake->mProfileId);
#else
	rcode = _tsnprintf(url, SAKE_MAX_URL_LENGTH, _T("http://%s.sake.gamespy.com/SakeFileServer/download.aspx?fileid=%d&gameid=%d&pid=%d"),
		sake->mGameName, fileId, sake->mGameId, sake->mProfileId);
#endif
...
#ifdef GSI_UNICODE
	// use capital %S to convert the gamename to a wide string
	rcode = _tsnprintf(url, SAKE_MAX_URL_LENGTH, _T("http://%S.sake.gamespy.com/SakeFileServer/upload.aspx?gameid=%d&pid=%d"),
		sake->mGameName, sake->mGameId, sake->mProfileId);
#else
	rcode = _tsnprintf(url, SAKE_MAX_URL_LENGTH, _T("http://%s.sake.gamespy.com/SakeFileServer/upload.aspx?gameid=%d&pid=%d"),
		sake->mGameName, sake->mGameId, sake->mProfileId);
#endif
