...
HOSTENT * getlocalhost(void)
{
	...
	SOGetInterfaceOpt(NULL, SO_SOL_CONFIG, SO_CONFIG_IP_ADDR_NUMBER, &aNumOfIps, &aSizeNumOfIps);
	
	aAddrs = (IPAddrEntry *)gsimalloc( aNumOfIps*sizeof(IPAddrEntry));
	SOGetInterfaceOpt(NULL, SO_SOL_CONFIG, SO_CONFIG_IP_ADDR_TABLE, &aNumOfIps, &aSizeNumOfIps);
	
	aLocalHost.h_name = "localhost";
	aLocalHost.h_aliases = &aliases;
	aLocalHost.h_addrtype = AF_INET;
	aLocalHost.h_length = (short)aNumOfIps;
	ips = (u8 **)gsimalloc(aNumOfIps * sizeof(u8 *));
