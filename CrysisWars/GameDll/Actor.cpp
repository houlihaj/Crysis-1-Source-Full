...
void CActor::SetDropWeaponTimer( CWeapon*pWeapon2Drop, float delay )
{
	if(delay<0.f)
	{
		if(pWeapon2Drop==m_pDropWpnPending)
			m_pDropWpnPending = NULL;
		return;
	}
	if(m_pDropWpnPending)
		m_pDropWpnPending->DoDrop();
	m_pDropWpnPending = pWeapon2Drop;
...
void CActor::Update(SEntityUpdateContext& ctx, int slot)
{
	...
	if(m_pDropWpnPending && (m_dropWpnWaitTime-=ctx.fFrameTime)<0.f)
	{
		m_pDropWpnPending->DoDrop();
		m_pDropWpnPending = NULL;
	}
