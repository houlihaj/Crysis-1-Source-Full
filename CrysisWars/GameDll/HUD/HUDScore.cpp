...
CHUDScore::ScoreEntry::ScoreEntry(EntityId id, int kills, int deaths, int ping, int teamKills): m_entityId(id), m_team(-1)
{
	m_kills = kills;
...
void CHUDScore::AddEntry(EntityId player, int kills, int deaths, int ping, int teamkills)
{
	...
	m_scoreBoard.push_back(ScoreEntry(player, kills, deaths, ping, teamkills));
}
...
void CHUDScore::Render()
{
	...
	for(it = m_scoreBoard.begin(); it != m_scoreBoard.end(); ++it)
	{
		...
		{
			int playerScore = player.m_kills*3 - player.m_deaths - player.m_teamKills*3;
