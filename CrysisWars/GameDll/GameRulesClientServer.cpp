...
IMPLEMENT_RMI(CGameRules, SvRequestSimpleHit)
{
	...
	if(!pActor)
 	...
 	if (pActor->GetEntityId()!=params.shooterId)
 	{
		char buf[256]={0};
		sprintf(buf, "INVALID SIMPLEHIT REQUEST 1:%d %2:%d", pActor?1:0, pActor->GetEntityId()!=params.shooterId?1:0);
		pNetChannel->Disconnect(eDC_PunkDetected, buf);
 		return true;
 	}
	...
	if (pActor->GetHealth()<=0)
 	...
 	if ((params.weaponId==params.shooterId || (!params.weaponId && params.type)) && (params.targetId && params.targetId!=pActor->GetEntityId()))
 	{
		char buf[256]={0};
		sprintf(buf, "INVALID SIMPLEHIT REQUEST 3:%d 4:%d 5:%d 6:%d 7:%d",
			params.weaponId==params.shooterId?1:0,
			params.weaponId,
			params.type,
			params.targetId,
			params.targetId!=pActor->GetEntityId()?1:0
		);
		pNetChannel->Disconnect(eDC_PunkDetected, buf);
 		return true;
 	}
...
IMPLEMENT_RMI(CGameRules, SvRequestHit)
{
	...
	if(!pActor)
 	...
 	{
		char buf[256]={0};
		sprintf(buf, "INVALID HIT REQUEST 1:%d 2:%d", pActor?1:0, pActor->GetEntityId()!=params.shooterId?1:0);
		pNetChannel->Disconnect(eDC_PunkDetected, buf);
 		return true;
 	}
	...
	if (pActor->GetHealth()<=0)
 	...
 	if ((params.weaponId==params.shooterId || !params.weaponId) && (params.targetId && params.targetId!=pActor->GetEntityId()))
 	{
		char buf[256]={0};
		sprintf(buf, "INVALID HIT REQUEST 3:%d 4:%d 5:%d 6:%d",
			params.weaponId==params.shooterId?1:0,
			params.weaponId,
			params.targetId,
			params.targetId!=pActor->GetEntityId()?1:0
