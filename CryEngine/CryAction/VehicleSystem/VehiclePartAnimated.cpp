...
//------------------------------------------------------------------------
bool CVehiclePartAnimated::Init(IVehicle* pVehicle, const SmartScriptTable &table, IVehiclePart* parent, CVehicle::SPartInitInfo& initInfo)
{
	SmartScriptTable animatedTable;
	if (!table->GetValue("Animated", animatedTable))
		return false;

	if (!CVehiclePartBase::Init(pVehicle, table, parent, initInfo))
		return false;

	ICharacterManager* pCharManager = GetISystem()->GetIAnimationSystem();

	char* pName;
	if (animatedTable->GetValue("filename", pName))
	{
		m_filename = pName;
		
    if (animatedTable->GetValue("filenameDestroyed", pName))		
			m_filenameDestroyed = pName;
	...
	if (animatedTable->GetValue("destroyedSuffix", pName))
		m_destroyedSuffix = pName;

  InitGeometry();
	InitHelpers(pVehicle, table);

	SmartScriptTable rotationParams;
	if (animatedTable->GetValue("Rotation", rotationParams))
...
//------------------------------------------------------------------------
void CVehiclePartAnimated::OnHit(Vec3 localPos, float radius, float strength)
{ 
  if (0 == VehicleCVars().v_deformable)
    return;
  ...
  for (uint32 j = 0; j < pSkeletonPose->GetJointCount(); j++)
  {
    if (IStatObj* pStatObj = pSkeletonPose->GetStatObjOnJoint(j))
    {
      AABB jointBounds = AABB::CreateTransformedAABB(Matrix34(pSkeletonPose->GetAbsJointByID(j)), pStatObj->GetAABB());

      if (jointBounds.IsOverlapSphereBounds(localPos, radius))
      {
        if (IStatObj* pStatObjNew = pStatObj->DeformMorph(Matrix34(pSkeletonPose->GetAbsJointByID(j).GetInverted()) * localPos, radius, strength))
        {
          if (pStatObjNew != pStatObj)
          { 
            pSkeletonPose->SetStatObjOnJoint(j, pStatObjNew);            
          }
        }
      }
    }
...
{	
  if (!m_filename.empty())
    ...
    if (!m_pCharInstanceDestroyed && !m_filenameDestroyed.empty())
      m_pCharInstanceDestroyed = GetISystem()->GetIAnimationSystem()->CreateInstance(m_filenameDestroyed.c_str());
...
IStatObj* CVehiclePartAnimated::GetDestroyedGeometry(const char* pJointName, unsigned int index)
{
    ...
    buffer[sizeof(buffer)-1] = '\0';
		...
		{
			IStatObj* pStatObj=pSkeletonDestroyed->GetStatObjOnJoint(jointIdForDestroyed);
			if(pStatObj)
			{
				static ICVar *pVar = gEnv->pConsole->GetCVar("sys_spec_Physics");
				if( pVar && pVar->GetRealIVal() == 1 && !strstr(pJointName, "hull"))
					return NULL;
				else
					return pStatObj;
			}
...
{ 
	if (m_rotatingAnimatedJoints.size() == 0)
	{
		if (m_pCharInstance)
		{
			...
			pSkeletonPose->SetPostProcessCallback0(	(int (*)(ICharacterInstance*,void*)) CVehiclePartAnimated_ProcessRotations, (void*)this);
		}
	}
