...
typedef std::map<IVehicleEventListener*, string> TVehicleEventListenerList;
...
public:
	...
	static const uint8 ASPECT_SEAT_PASSENGER	= eEA_GameServerStatic;
	static const uint8 ASPECT_SEAT_ACTION	= eEA_GameClientDynamic;
	static const uint8 ASPECT_COMPONENT_DAMAGE = eEA_GameServerStatic;
	static const uint8 ASPECT_PART_MATRIX	= eEA_GameClientDynamic;
	static const uint8 ASPECT_FROZEN = eEA_GameServerStatic;
  ...
  virtual void SetAmmoCapacity(IEntityClass* pAmmoType, int capacity);
	...
	virtual void SendAmmoCounts(int where, int channelId=-1);
  ...
  virtual IVehiclePart* AddPart(const SmartScriptTable& partParams, IVehiclePart* parent, SPartInitInfo& initInfo);
  ...
  virtual bool HasHelper(const char* name);
	...
	virtual bool SetMovement(const string& movementName, const SmartScriptTable &table);
  ...
  virtual void SetVehicleGlassSurfaceType(ISurfaceType* pSurface) { sSurfaceMatGlassVehicle=pSurface; }  
	...
	void SetWindowMaterial(int partid, IMaterial *pMat);
	...
	struct AmmoCountsParams
	{
		...
		{
			AmmoCount() {};
			AmmoCount(const char *n, int c): name(n), count(c) {};

			string	name;
			int			count;
			void SerializeWith(TSerialize ser)
			{
				ser.Value("AmmoName", name, 'stab');
				ser.Value("AmmoCount", count, 'ammo');
			}
		};

		std::vector<AmmoCount> ammoCounts;
		...
		void SerializeWith(TSerialize ser)
		{
			...
			{
				int nammos=0;
				ser.Value("NumberAmmos", nammos, 'vNWp');
				ammoCounts.reserve(nammos);
				for (int i=0;i<nammos;i++)
				{
					ammoCounts.push_back(AmmoCount());
					ammoCounts.back().SerializeWith(ser);
				}
			}
			else
			{
				int nammos=(int)ammoCounts.size();
				ser.Value("NumberAmmos", nammos, 'vNWp');
				for (int i=0;i<nammos;i++)
					ammoCounts[i].SerializeWith(ser);
			}
		};
	};
...
public:
	...
	bool AddComponent(const SmartScriptTable &componentParams);
  ...
  float GetSelfCollisionMult(const Vec3& velocity, const Vec3& normal, int partId, EntityId colliderId);
	float GetPlayerCollisionMult() const { return m_damageParams.playerCollisionMult; }
  ...
  const char* GetSoundName(const char* eventName);
	...
	bool InitRespawn();
	void OnDestroyed(EntityId shooterId=0);
...
protected:
	...
	bool InitActions(const SmartScriptTable& vehicleTable);
  void InitPaint(const SmartScriptTable& xmlContent);
	bool IsActionUsable(const SVehicleActionInfo& actionInfo, EntityId userId);
  
	void LoadParts(SmartScriptTable& table, IVehiclePart* pParent, SPartInitInfo& initInfo);
  ...
  bool InitParticles(const SmartScriptTable& table);  
  void InitModification(XmlNodeRef data, const char *modification);
  ...
  TVehiclePartSet m_physicalizeParts;
	...
	std::vector<int> m_brokenWindowMaterial;
	std::vector<int> m_ser_brokenWindows;
