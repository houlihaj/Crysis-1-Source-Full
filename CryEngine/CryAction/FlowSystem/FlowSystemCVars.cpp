...
namespace
{
	...
	void InspectAction(IConsoleCmdArgs* args)
	{
		...
		if (!isAnyAction && gEnv->pAISystem->GetAIAction(actionName) == 0)
		{
...
CFlowSystemCVars::CFlowSystemCVars()
{
	...
	pConsole->AddCommand("fg_Inspector", Inspector, 0,
		"Toggles FlowGraph System DefaultInspector.\n"
		"Usage: fg_Inspector [0/1]\n"
		"Default is 0 (off).");
	pConsole->AddCommand("fg_InspectEntity", InspectEntity, 0, "Inspects the specified Entity graph");
	pConsole->AddCommand("fg_InspectAction", InspectAction, 0, "Inspects the specified AIAction graph");
	pConsole->Register("fg_SystemEnable", &this->m_enableUpdates, 1, VF_CHEAT, "Toggles FlowGraph System Updates.\nUsage: fg_SystemEnable [0/1]\nDefault is 1 (on).");
	pConsole->Register("fg_profile", &this->m_profile, 0, 0, "Flow graph profiling enable/disable");
	pConsole->Register("fg_abortOnLoadError", &this->m_abortOnLoadError, 1, 0, "Abort on load error of flowgraphs\n2:abort, 1:dialog, 0:log only");
	pConsole->Register("fg_inspectorLog", &this->m_inspectorLog, 0, 0, "Log inspector on console.");
	pConsole->Register("fg_noDebugText", &this->m_noDebugText, 0, 0, "Don't show debug text [0/1]\nDefault is 0 (show debug text).");
}
