...
void CClassRegistryReplicator::GetMemoryStatistics(ICrySizer * s)
{
	s->AddContainer(m_classNameToId);
	s->AddContainer(m_classIdToName);
	for (size_t i=0; i<m_classIdToName.size(); i++)
	{
		s->Add(m_classIdToName[i]);
	}
	for (std::map<string, uint16>::iterator iter = m_classNameToId.begin(); iter != m_classNameToId.end(); ++iter)
		s->Add(iter->first);
}