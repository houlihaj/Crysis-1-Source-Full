...
public:
	...
	EContextEstablishTaskResult OnStep( SContextEstablishState& state )
	{
		...
		uint16 id = ~uint16(0);
		if (!m_pRep->ClassIdFromName(id, CCryAction::GetCryAction()->GetGameContext()->GetRequestedGameRules()) || id == (uint16)(~uint16(0)))
		{
		...
		uint64 checkSum = CCryAction::GetCryAction()->GetGameContext()->GetLevelChecksum();
		ICVar* pURL = gEnv->pConsole->GetCVar("net_mapDownloadURL");
		state.pSender->AddSendable( 
			new CSimpleNetMessage<SGameTypeParams>(SGameTypeParams(id, CCryAction::GetCryAction()->GetGameContext()->GetLevelName(), CCryAction::GetCryAction()->GetGameContext()->HasContextFlag(eGSF_ImmersiveMultiplayer), checkSum, (pURL != NULL) ? pURL->GetString() : ""), CGameClientChannel::SetGameType), 
...
public:
	...
	EContextEstablishTaskResult OnStep( SContextEstablishState& state )
	{
		...
		if(!loadingSaveGame && !GetISystem()->IsSerializingFile())
		{
			...
			CCryAction::GetCryAction()->SaveGame(levelstart.c_str(), true);
		}
