...
#define MAX_BOID_HEIGHT_SURVIVE						2.5f

//////////////////////////////////////////////////////////////////////////
CChickenFlock::CChickenFlock( CEntity *pEntity )
...
void CChickenFlock::CreateBoids( SBoidsCreateContext &ctx )
{
	...
	for (int i = 0; i < m_RequestedBoidsCount; i++)
	{
		...
		boid->m_planeHeight = m_origin.z;
		boid->m_pos = m_origin + Vec3(radius*frand(),radius*frand(),frand()*radius);
		boid->m_pos.z = (m_bc.bPlanarPosition ? boid->m_planeHeight : m_bc.engine->GetTerrainElevation(boid->m_pos.x,boid->m_pos.y)) + m_bc.fBoidRadius*0.5f;
...
//////////////////////////////////////////////////////////////////////////
void CChickenFlock::OnAIEvent(EAIEventType type, const Vec3& pos, float radius, float threat, EntityId sender)
{
...
void CChickenBoid::OnPickup( bool bPickup,float fSpeed )
{
	...
	else
	{
		...
		if (!m_dead)
		{
			if (fSpeed > 5.0f || ForceKillOnDrop())
			{
...
void CChickenBoid::Update( float dt,SBoidContext &bc )
{
	...
	if (bc.followPlayer)
	{
		if (m_pos.GetSquaredDistance(bc.playerPos) > MAX_CHICKEN_DISTANCE_FROM_PLAYER*MAX_CHICKEN_DISTANCE_FROM_PLAYER)
		{
			...
			m_pos.z = (bc.bPlanarPosition ? m_planeHeight : bc.engine->GetTerrainElevation(m_pos.x,m_pos.y)) + bc.fBoidRadius*0.5f;
			m_speed = bc.MinSpeed;
...
bool CChickenBoid::ForceKillOnDrop()
{
	if (m_flock->m_bc.bPlanarPosition)
	{
		Vec3 flpos=m_flock->GetPos();
		float d2d=m_pos.GetSquaredDistance2D(flpos);
		bool kill= d2d > sqr(m_flock->m_bc.fSpawnRadius) || abs(m_pos.z-flpos.z) > MAX_BOID_HEIGHT_SURVIVE;
		//gEnv->pLog->Log(">>>>>>>>>>> Boid forced to kill %d : %f :%f : %f", kill ? 1 : 0, sqrt(d2d), m_flock->m_bc.fSpawnRadius, abs(m_pos.z-flpos.z));
		return kill;
	}
...
//////////////////////////////////////////////////////////////////////////
CTurtleFlock::CTurtleFlock( CEntity *pEntity ) : CChickenFlock(pEntity)
{
