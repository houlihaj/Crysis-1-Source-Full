...
{
	float fLenSqr = v.x*v.x + v.y*v.y + v.z*v.z;
	//	assert(fLen>0.0001f);
	unsigned int *n1 = (unsigned int *)&fLenSqr;
...
void CBoidObject::PlayAnimation( const char *animation,bool bLooped,float fBlendTime )
{
	if (m_object)
	{
		...
		m_object->GetISkeletonAnim()->StartAnimation( animation ,0, 0,0,  animParams );
...
{
	if (nIndex >= 0 && nIndex < m_flock->m_bc.animations.size())
...
{
	if (nIndex >= 0 && nIndex < m_flock->m_bc.sounds.size())
	{
...
int CBoidObject::GetGeometrySurfaceType()
{
	...
	if (m_object)
	{
		ISurfaceType *pSurfaceType = ((CBreakableManager*)g_pIEntitySystem->GetBreakableManager())->GetFirstSurfaceType(m_object);
		if (pSurfaceType)
	...
	else
	{
		...
		if (pEntity)
		{
			...
			if (pStatObj)
			{
				ISurfaceType *pSurfaceType = ((CBreakableManager*)g_pIEntitySystem->GetBreakableManager())->GetFirstSurfaceType(pStatObj);
				if (pSurfaceType)
					return pSurfaceType->GetId();
			}
		}
...
void CBoidObject::OnCollision( SEntityEvent &event )
{
	...
	// Prevent lethal boid-boid collisions!!!
	if (pPhysObject)
	{
		CEntity *pEntity = (CEntity*)pPhysObject->GetForeignData(PHYS_FOREIGN_ID_ENTITY);
		if (pEntity && pEntity->GetProxy(ENTITY_PROXY_BOID_OBJECT))
			return;
...
void CBoidBird::Think( float dt,SBoidContext &bc )
{
	...
	// Gaussian weight.
	m_accel.z = cry_expf(dhexp) * bc.factorKeepHeight;
	...
	if (m_physicsControlled)
	{
		if (m_pPhysics)
		{
...

			{
				...
				// Apply force on this body.
				pe_action_impulse theAction;
				theAction.impulse = Vec3(sin(m_dyingTime*0.1f),cos(m_dyingTime*0.13f),cos(m_dyingTime*0.171f)*2.8f) * 0.01f;
				...
				pe_simulation_params sym;
				sym.density = 950.0f + 200.0f*sin(m_dyingTime);
...
//////////////////////////////////////////////////////////////////////////
CFlock::CFlock( CEntity *pEntity,EFlockType flockType )
{
...
{
	if (m_pEntity && m_pEntity->GetRenderProxy())
		m_pEntity->GetRenderProxy()->FreeAllSlots();
	...
	for (Boids::iterator it = m_boids.begin(); it != m_boids.end(); ++it)
	{
		...
		if (boid->m_entity)
		{
			...
			if (pBoidEntity)
			{
				CBoidObjectProxy *pBoidObjectProxy = (CBoidObjectProxy*)pBoidEntity->CreateProxy(ENTITY_PROXY_BOID_OBJECT);
				if (pBoidObjectProxy)
...
void CFlock::Update( CCamera *pCamera )
{
	...
	*/
	...
	m_bc.randomFlockCenter = m_bc.flockPos + 
		//m_bc.fSpawnRadius*Vec3(sin(0.9f*m_fCenterFloatingTime),cos(1.1f*sin(0.9f*m_fCenterFloatingTime)),0.3f*sin(1.2f*m_fCenterFloatingTime) );
		m_bc.fSpawnRadius*Vec3(sin(tc*0.913)*cos(tc*1.12f),sin(tc*0.931f)*cos(tc*0.971f),0.4f*sin(tc*1.045)*cos(tc*0.962f) );
...
void CFlock::RegisterAIEventListener( bool bEnable )
{
	...
	if (bEnable)
	{
		...
		gEnv->pAISystem->RegisterAIEventListener(this,m_bc.flockPos,m_bc.maxVisibleDistance,AIEVENT_EXPLOSION|AIEVENT_BULLET );
	}
...
//////////////////////////////////////////////////////////////////////////
void CFlock::OnAIEvent(EAIEventType type, const Vec3& pos, float radius, float threat, EntityId sender)
{
