...
void CReverbInstance_RoomMachine844::FmodErrorOutput(const char * sDescription, IMiniLog::ELogType LogType)
{
	switch(LogType) 
	{
...
bool CReverbInstance_RoomMachine844::Activate()
{
	...
	if (m_pCSEX && !m_DSPReverb)
	{
		...
		{
			FmodErrorOutput("load /Libs/ReverbPlugins/VST_RoomMachine844.dll reverb failed! ", IMiniLog::eError);
			return false;
		}
		...
		if (m_ExResult != FMOD_OK)
		{
			FmodErrorOutput("create DSP by index failed! ", IMiniLog::eError);
			return false;
		}
		...
		if (m_DSPReverb)
		{
			...
			if (m_ExResult != FMOD_OK)
			{
				FmodErrorOutput("add reverb to DSP chain failed! ", IMiniLog::eError);
				return false;
			}
			...
			if (m_ExResult != FMOD_OK)
			{
				FmodErrorOutput("activate reverb failed! ", IMiniLog::eError);
				return false;
			}
