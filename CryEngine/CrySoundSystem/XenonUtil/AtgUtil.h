...
//--------------------------------------------------------------------------------------
VOID CDECL DebugSpew( const CHAR*, ... ) PRINTF_PARAMS(1, 2);  // Un-modified debug spew
VOID CDECL FatalError( const CHAR*, ... ) PRINTF_PARAMS(1, 2); // Debug spew with a forced break
...
public:
    ...
    WCHAR* GetFrameRate()
    {
        ...
        if( fTime - m_fLastFPSTime > 1.0 )
        {
            ...
            m_fLastFPSTime = fTime;
            m_dwNumFrames  = 0L;
            swprintf( m_strFrameRate, L"%0.02f fps", (FLOAT)fFPS );
        }
...
//--------------------------------------------------------------------------------------
VOID GetVideoSettings( UINT* pdwBackBufferWidth, UINT* pdwBackBufferHeight );
