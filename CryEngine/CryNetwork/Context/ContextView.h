...
public:
	...
	bool IsServer() const { return !IsClient(); }
  ...
  bool HasWitness( EntityId id )
  {
	...
	void SetAspectProfile( SNetObjectID objectID, uint8 aspect, uint8 profile );
  ...
  // is this a local view? (it communicates with another view that is 
	...
	bool ScheduleAttachment( IRMIMessageBodyPtr pMessage, const SAttachmentIndex * pIndex = NULL );
...
protected:
	...
	bool ClearAspects( SNetObjectID netID, uint8 aspects );
...
#if ENABLE_ASPECT_HASHING
	void HashAspect( uint8 aspect, TSerialize ser );
#endif
  ...
  void DefineExtensionsProtocol(IProtocolBuilder*);
	...
	void PolluteObjectAspect( SNetObjectID netID, uint8 aspectIdx );
	void PartialAspect( uint8 aspect, TSerialize ser );
...
private:
	...
	// bind an aspect
	void BindAspects( SNetObjectID obj, uint8 aspects );
	void UnbindAspects( SNetObjectID obj, uint8 aspects );
...
protected:
	uint8 GetSentAspects( SNetObjectID, bool assumeEnabled, bool * assumeAuthority );
	...
	bool UpdateAspect( int i, TSerialize pSerialize, uint32 nCurSeq, uint32 nOldSeq );
	SReceiveContext CreateReceiveContext( TSerialize ser, uint8 index, uint32 nCurSeq, uint32 nOldSeq, bool * ok );
	...
	void ChangedObject( SNetObjectID id, uint32 flags, uint8 dirtyAspects );
...
private:
	...
	void CancelUpdates();
	void PrimeUpdates();
	void NotifyPartialUpdate( SNetObjectID id, uint8 aspect );
...
private:
  ...
  TExtensionsSet m_extensions;
	...
	typedef std::map<SNetObjectID, uint8, std::less<SNetObjectID>, STLMementoAllocator<std::pair<SNetObjectID, uint8> > > TEarlyPartialUpdateIDs;
...
public:
	...
	CContextViewObjectLock( const CContextViewObjectLock& lk ) : m_pContext(lk.m_pContext), m_objId(lk.m_objId)
	{
		if (m_objId)
			m_pContext->IncObjLock(m_objId);
	}
