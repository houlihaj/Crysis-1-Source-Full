...
struct SControllerArray
{

#pragma warning(disable:4200) // nonstandard extension used : zero-sized array in struct/union

	struct SControllerArrayInternal
	{
		void AddRef()
		{
			++m_nRefCounter;
		...
		int m_nRefCounter;
		class IController *m_Controllers[];
	};

#pragma warning(default:4200)

	_smart_ptr<SControllerArrayInternal> m_Array;

	IController *&operator[](int index) const
	{
		return m_Array->m_Controllers[index];
	}

	void resize(int newsize)
	{
		assert(m_Array == NULL);
		size_t array_size = sizeof(SControllerArrayInternal) + sizeof(IController *) * newsize;
		uint8 *ptr = new uint8[array_size];
		memset(ptr, 0, array_size);
		m_Array = reinterpret_cast<SControllerArrayInternal *>(ptr);
	}
};
...
struct SLocoGroup
{
	...
	SControllerArray m_jointControllers[MAX_LMG_ANIMS];
