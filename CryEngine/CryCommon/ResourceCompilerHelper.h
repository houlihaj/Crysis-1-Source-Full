...
public:
	...
	CResourceCompilerHelper(const char* moduleName=NULL);
	...
	string ProcessIfNeeded( const char *szFilePath, const bool bReload )
	{
		...
		for(uint32 dwIndex=0;;++dwIndex)		// check for all input files
		{
			...
			{
				// call rc.exe
				if(!InvokeResourceCompiler(sSrcFile.c_str(),sDestFile.c_str(),PathUtil::GetGameFolder(),false))		// false=no window
				{
					m_bErrorFlag=true;
	...
	string GetRootPath(bool pullFromRegistry);
	...
	bool CallResourceCompiler( const char *szFileName=NULL, const char *szAdditionalSettings=NULL, IResourceCompilerListener* listener = 0, bool mayShowWindow=true, bool bUseQuota=true, bool pullFromRegistry=true, bool silent=false,bool boNoUserDialog=false,const char *szWorkingDirectory=NULL);

	void* CallEditor( void* hParent, const char * pWndName, const char * pFlag );
...
private:
	...
	string GetInputFilename( const char *szFilePath, const unsigned int dwIndex ) const;	
