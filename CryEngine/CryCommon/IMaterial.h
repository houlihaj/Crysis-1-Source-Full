...
#define MTL_SPECIAL_NAME_VEHICLE_COLLISION_PROXY "mat_wood_vehicleonly"
...
enum EMaterialFlags
{
  ...
  MTL_FLAG_NON_REMOVABLE= 0x10000,  // Material with this flag once created are never removed from material manager (Used for decal materials, this flag should not be saved).
	...
	MTL_FLAG_VISIBLE_ONLY_IN_EDITOR = 0x80000,	// This material is visible only in editor mode (collision proxies).

	MTL_FLAG_FORCESHADOWBIAS = 0x100000, // Force using shadow bias
};

#define MTL_FLAGS_SAVE_MASK (MTL_FLAG_WIRE|MTL_FLAG_2SIDED|MTL_FLAG_ADDITIVE|MTL_FLAG_ADDITIVE_DECAL|MTL_FLAG_LIGHTING|\
				MTL_FLAG_NOSHADOW|MTL_FLAG_PURE_CHILD|MTL_FLAG_MULTI_SUBMTL|MTL_FLAG_SCATTER|MTL_FLAG_TRANSLUCENCE|MTL_FLAG_HIDEONBREAK|MTL_FLAG_UIMATERIAL|MTL_FLAG_FORCESHADOWBIAS)
...
struct IMaterial
{
  ...
  virtual bool IsSubSurfScatterCaster() const = 0;
	virtual float GetPSRefractionAmount() const = 0;
