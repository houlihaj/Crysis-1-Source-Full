...
enum AnimParamType
{
	...
	APARAM_SHAKEAMPA,	//!< Camera shake amplitude A
	APARAM_SHAKEAMPB, //!< Camera shake amplitude B
	APARAM_SHAKEFREQA,//!< Camera shake frequency A
	APARAM_SHAKEFREQB,//!< Camera shake frequency B
	APARAM_SHAKEMULT, //!< Camera shake amplitude/frequency multipliers
	APARAM_SHAKENOISE,//!< Camera shake noise multipliers
	APARAM_SHAKEWORKING,//!< Camera shake time offsets and shake enabled/disabled
...
public:
	...
	*/
	...
	//! Evaluate animation node while not playing animation / parent synchronization
	virtual void SyncUpdate( ESyncUpdateType type,SAnimContext &ac ) = 0;
...
struct IAnimSequence : public _i_reference_target_t
{
	...
	enum Flags
	{
		PLAY_ONRESET	= 0x0001,	//!< Start playing this sequence immediately after reset of movie system(Level load).
		ORT_CONSTANT	= 0x0002,	//!< Constant Out-Of-Range,time continues normally past sequence time range.
		ORT_LOOP			= 0x0004,	//!< Loop Out-Of-Range,time wraps back to the start of range when reaching end of range.
		CUT_SCENE			= 0x0008,	//!< Cut scene sequence.
		NO_HUD				= 0x0010,	//!< Don`t display HUD
		NO_PLAYER			= 0x0020,	//!< Disable input and drawing of player
		NO_PHYSICS		= 0x0040,	//!< Don`t update physics
		NO_AI					= 0x0080,	//!< Don`t update AI
		IS_16TO9			= 0x0100,	//!< 16:9 bars in sequence
		NO_GAMESOUNDS	= 0x0200,	//!< Suppress all game sounds.
		NO_SEEK       = 0x0400, //!< Cannot seek in sequence. 
		NO_ABORT      = 0x0800, //!< Cutscene can not be aborted
		NO_TRIGGERS   = 0x1000, //!< Disable proximity/area triggers
	};
	...
	//! Get animation name.
	virtual const char* GetName() = 0;
	//! Set animation music pattern name.
	virtual void SetMusicPatternName( const char *musicPatternName ) = 0;
	//! Get animation music pattern name.
	virtual const char* GetMusicPatternName() = 0;
	...
	*/
	...
	/** Update sequence while not playing animation / parent synchronization
	*/
	virtual void SyncUpdate( ESyncUpdateType type,SAnimContext &ac ) = 0;
...
struct IMovieSystem
{
	...
	*/
	virtual void PlaySequence( const char *sequence,IAnimSequence *parentSeq,bool bResetFX,bool bTrackedSequence,float currentTime = -FLT_MAX,float startTime = -FLT_MAX,float endTime = -FLT_MAX ) = 0;

	/** Start playing sequence.
			Call ignored if sequence is already playing.
			@param sequence Pointer to Valid sequence to play.
	*/
	virtual void PlaySequence( IAnimSequence *seq,IAnimSequence *parentSeq,bool bResetFX,bool bTrackedSequence,float currentTime = -FLT_MAX,float startTime = -FLT_MAX,float endTime = -FLT_MAX ) = 0;
	...
	*/
	...
	// Sync sequence to a given time
	virtual bool SyncSequence(IAnimSequence *pSeq,ESyncUpdateType syncUpdateType,float fTime) = 0;
};
