...
//#define ERF_UPDATE_IF_PV								0x1000		//Not used
#define ERF_VISIBLE_ONLY_IN_EDITOR			0x1000
...
#define ERF_CASTSCATTERMAPS             0x8000000
...
struct IRenderNode : public IShadowCaster
{
  ...
  virtual void GetLocalBounds( AABB &bbox ) { AABB WSBBox(GetBBox()); bbox = AABB(WSBBox.min - GetPos(true), WSBBox.max - GetPos(true)); }
	...
	virtual IMaterial * GetEntitySlotMaterial( unsigned int nPartId ) { return NULL; }
  ...
  { 
	...
	// Summary:
	//   Assigns a vertex stream for per vertex scattering
	virtual void SetVertexScattering(const struct SScatteringObjectData& ScatteringObjectData) {}
  ...
  virtual int GetEditorObjectId() const { return 0; }
...
struct IVoxelObject : public IRenderNode
{
  ...
  virtual bool IsEmpty() = 0;
	virtual void MoveContent(const Vec3 &move) = 0;
