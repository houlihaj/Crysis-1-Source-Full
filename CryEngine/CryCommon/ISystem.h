...
struct SSystemInitParams
{
	...
	bool bIsUIApplication;											// When runing in Editor mode.
...
{ 
  ...
  // dumps the memory usage statistics to the log. default MB (can be KB)
	...
	// returns true if this is a ui application, other than the editor
	virtual bool IsUIApplication() {return false;}
  ...
  virtual IPhysicalWorld *GetIPhysicalWorld() = 0;
	...
	*/
	...
	virtual IEngineSettingsManager* GetEngineSettingsManager() = 0;

#ifdef USING_UNIKEY_SECURITY
	virtual IUniKeyManager *GetUniKeyManager() = 0;
#endif // USING_UNIKEY_SECURITY

#ifdef USING_LICENSE_PROTECTION
	virtual IProtectionManager *GetProtectionManager() = 0;
#endif // USING_LICENSE_PROTECTION
};
...
#endif EXCLUDE_CVARHELP

// the following macros allow the help text to be easily stripped out
// preferred way to register a CVar
#define REGISTER_CVAR(_var,_def_val,_flags,_comment)	gEnv->pConsole->Register((#_var), &(_var), (_def_val), (_flags), CVARHELP(_comment))
// preferred way to register a string CVar
#define REGISTER_STRING(_name,_def_val,_flags,_comment)	gEnv->pConsole->RegisterString(_name,(_def_val), (_flags), CVARHELP(_comment))
// preferred way to register a int CVar
#define REGISTER_INT(_name,_def_val,_flags,_comment)	gEnv->pConsole->RegisterInt(_name,(_def_val), (_flags), CVARHELP(_comment))
// preferred way to register a int CVar
#define REGISTER_FLOAT(_name,_def_val,_flags,_comment)	gEnv->pConsole->RegisterFloat(_name,(_def_val), (_flags), CVARHELP(_comment))
// offers more flexibility but more code is required
#define REGISTER_CVAR2(_name,_var,_def_val,_flags,_comment)	gEnv->pConsole->Register(_name, _var, (_def_val), (_flags), CVARHELP(_comment))
// preferred way to register a console command
#define REGISTER_COMMAND(_name,_func,_flags,_comment)	gEnv->pConsole->AddCommand(_name,_func,(_flags), CVARHELP(_comment))
