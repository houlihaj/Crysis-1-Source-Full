...
*/
  virtual IEntity* GetEntity( EntityId id ) const = 0;
	...
	// discards the salt, checks the entity slot, and returns the entity id of an entity existing in that slot (or zero if nothing is present)
	virtual EntityId GetEntityIdInSlot( EntityId id ) const = 0;
...
*/
	...
	virtual CAreaManager* GetAreaManager() const = 0;
 	virtual bool IsInDiagArea( const Vec3& pos ) = 0;
