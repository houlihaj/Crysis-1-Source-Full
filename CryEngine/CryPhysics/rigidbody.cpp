...
inline bool should_swap(entity_contact **pContacts,int i1,int i2,int &diff) { // sub-sorts contacts by pbody[1]
...
inline bool should_swap(RigidBody **pBodies, int i1,int i2,int &diff) { // sorts bodies by level
...
bool should_swap(entity_contact_unproj **pContacts,int i1,int i2,int &diff) { // sorts contacts by newly sorted bodies 
	...
	diff |= g_infos[pContacts[i1]->pbody[iop1]->bProcessed].idx-g_infos[pContacts[i2]->pbody[iop2]->bProcessed].idx;
...
void InvokeContactSolver(float time_interval, SolverSettings *pss, float Ebefore)
{
	...
	int i,j,iop,nBodies,istart,iend,istep,iter,bBounced,nBounces,bContactBounced,nConstraints,nMaxIters,nBuddies,bNeedLCPCG=0;
...
__solver_step++;
	...
	for(i=nBodies=nConstraints=0; i<g_nContacts; i++) {
		...
		else for(iop=0,prope=(rope_solver_vtx*)g_pContacts[i]->pBounceCount; iop<g_pContacts[i]->iCount; iop++) {
...
//bBounced=1; if (false)
			...
			for(i=istart; i!=iend; i+=istep) {
				if (g_ContactsRB[i].iCount >= (g_ContactsRB[i].flags & contact_count_mask))	{
					...
					if (g_ContactsRB[i].flags & contact_constraint) {
						if ((g_pContacts[i]->C*dp).len2() > max(sqr(e),g_ContactsRB[i].vreq.len2()*sqr(0.05f))) {
						...
						if ((vrel=dp*n)<0 &&
								(isneg(e-fabs_tpl(vrel)) | isneg(0.0001f-g_ContactsRB[i].Pspare) & isneg(sqr(pss->minSeparationSpeed)-(dp-n*vrel).len2()))) 
					...
					} else if (dp.len2()>sqr(pss->minSeparationSpeed) && g_ContactsRB[i].Pspare>g_pContacts[i]->pbody[0]->M*0.001f) {
						dP = dp*(-dp.len2()/(dp*g_ContactsRB[i].K*dp));
						g_ContactsRB[i].Pn += dPn=dP*n;
		...
		for(i=bBounced=0; i<g_nContacts; i++) {
			...
			if (g_pContacts[i]->flags & contact_rope) {
				...
				for(iop=0,g_pContacts[i]->vrel.zero(); iop<2; iop++) {
					...
					g_pContacts[i]->vrel.x += g_pContacts[i]->C.GetRow(iop)*(body0->Fcollision*body0->Minv + (body0->Iinv*body0->Tcollision^r0));
				} for(iop=0,prope=(rope_solver_vtx*)g_pContacts[i]->pBounceCount; iop<g_pContacts[i]->iCount; iop++) {
					body0 = prope[iop].pbody;
					g_pContacts[i]->vrel.x += prope[iop].v*(body0->Fcollision*body0->Minv + (body0->Iinv*body0->Tcollision^prope[iop].r));
				}
		...
		if (bBounced) {
			...
			do {
				...
				for(i=0,r2=0; i<g_nContacts; i++) {
					...
					g_pContacts[i]->dP = iClass-1u<4u ? g_pContacts[i]->n*(g_pContacts[i]->dPn=g_pContacts[i]->vrel.x) : g_pContacts[i]->vrel;
					g_pContacts[i]->P.zero();
				}
				...
				if (r2>0 && iter>0) do {
					...
					for(i=0;i<nAngContacts;i++) { // angular contacts
						...
						for(iop=0,dPn=pContacts[i]->dPn; iop<2; iop++,dPn*=pContacts[i]->K(0,0)) {
							body0 = pContacts[i]->pbody[iop]; dP = pContacts[i]->C.GetRow(iop)*dPn;
							body0->Fcollision += dP; body0->Tcollision += pContacts[i]->pt[iop]-body0->pos^dP;
						}	for(iop=0,prope=(rope_solver_vtx*)pContacts[i]->pBounceCount; iop<pContacts[i]->iCount; iop++) {
							prope[iop].pbody->Fcollision += prope[iop].P*pContacts[i]->dPn;
							prope[iop].pbody->Tcollision += prope[iop].r^prope[iop].P*pContacts[i]->dPn;
						}
					} for(;i<nContacts;i++) { // positional contacts
					...
					for(i=0;i<nAngContacts;i++) {	// angular contacts
						...
						for(iop=0,pContacts[i]->vrel.zero(); iop<2; iop++) {
							body0 = pContacts[i]->pbody[iop]; r0 = pContacts[i]->pt[iop]-body0->pos;
							pContacts[i]->vrel.x += pContacts[i]->C.GetRow(iop)*(body0->Fcollision*body0->Minv + (body0->Iinv*body0->Tcollision^r0));
						} for(iop=0,prope=(rope_solver_vtx*)pContacts[i]->pBounceCount; iop<pContacts[i]->iCount; iop++) {
							body0 = prope[iop].pbody;
							pContacts[i]->vrel.x += prope[iop].v*(body0->Fcollision*body0->Minv + (body0->Iinv*body0->Tcollision^prope[iop].r));
						}
					} for(;i<nContacts;i++) { // positional contacts
						body0 = pContacts[i]->pbody[0]; body1 = pContacts[i]->pbody[1];
						r0 = pContacts[i]->pt[0]-body0->pos; r1 = pContacts[i]->pt[1]-body1->pos;
					...
					a = min((real)50.0,r2/max((real)1E-10,max((real)1E-10,pAp))); r2new=0;
				...
				for(i=0;i<nAngContacts;i++) { // angular contacts
					...
					for(iop=0,dPn=pContacts[i]->P.x; iop<2; iop++,dPn*=pContacts[i]->K(0,0)) {
						body0 = pContacts[i]->pbody[iop]; dP = pContacts[i]->C.GetRow(iop)*dPn;
						body0->Fcollision += dP; body0->Tcollision += pContacts[i]->pt[iop]-body0->pos^dP;
					}	for(iop=0,prope=(rope_solver_vtx*)pContacts[i]->pBounceCount; iop<pContacts[i]->iCount; iop++) {
						prope[iop].pbody->Fcollision += prope[iop].P*pContacts[i]->P.x;
						prope[iop].pbody->Tcollision += prope[iop].r^prope[iop].P*pContacts[i]->P.x;
					}
				} for(;i<nContacts;i++) { // positional contacts
					body0 = pContacts[i]->pbody[0]; body1 = pContacts[i]->pbody[1]; 
					r0 = pContacts[i]->pt[0]-body0->pos; r1 = pContacts[i]->pt[1]-body1->pos;
			...
			if (cgiter<pss->nMaxLCPCGiters) {
				...
				if (bBounced) {
					...
					for(i=0;i<nAngContacts;i++) { // angular contacts
						...
						for(iop=0,dPn=pContacts[i]->P.x; iop<2; iop++,dPn*=pContacts[i]->K(0,0)) {
							body0 = pContacts[i]->pbody[iop]; dP = pContacts[i]->C.GetRow(iop)*dPn;
							body0->P += dP; body0->L += pContacts[i]->pt[iop]-body0->pos^dP;
						}	for(iop=0,prope=(rope_solver_vtx*)pContacts[i]->pBounceCount; iop<pContacts[i]->iCount; iop++) {
							prope[iop].pbody->P += prope[iop].P*pContacts[i]->P.x;
							prope[iop].pbody->L += prope[iop].r^prope[iop].P*pContacts[i]->P.x;
						}
					} for(;i<nContacts;i++) { // positional contacts
						body0 = pContacts[i]->pbody[0]; body1 = pContacts[i]->pbody[1]; 
						r0 = pContacts[i]->pt[0]-body0->pos; r1 = pContacts[i]->pt[1]-body1->pos;
	...
	for(i=0,r2=r2new=0;i<nAngContacts;i++) {
		...
		iop = pContacts[i]->flags>>contact_bidx_log2&1;
		r2 += sqr(pContacts[i]->r0.x);
