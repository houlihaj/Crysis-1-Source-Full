...
#define	M_2PI		(2.0f*3.14159f - 0.00001f)
	...
	****************************************************************************/
	...
	class	TCBAngleAxisSpline : public TCBSpline<Quat,TCBAngAxisKey>
	{
	public:
		//void interpolate( float time,value_type& val );
		void comp_deriv();
	...
	inline	void	TCBAngleAxisSpline::interp_keys( int from,int to,float u,value_type& val )
	{
		u = calc_ease( u,key(from).easefrom,key(to).easeto );
		basis_type basis( u );
		val = Quat::CreateSquadRev( angle(to),axis(to), value(from), dd(from), ds(to), value(to), u );
		val = (val).GetNormalized();	// Normalize quaternion.
	}
	...
	inline	void TCBAngleAxisSpline::compKeyDeriv( int curr )
	{
		...
		if (curr > 0 || closed())
		{
			...
			if (angle(curr) > M_2PI) {
				Vec3 a = axis(curr);
				qm = Quat(0,log( Quat(0,a.x,a.y,a.z) ));
			} else {
				qm = value(prev);
		...
		if (curr < last || closed())
		{
			...
			if (angle(next) > M_2PI) {
				Vec3 a = axis(next);
				qp = Quat(0,log(Quat(0,a.x,a.y,a.z)));
			} else {
				Quat qnext = value(next);
