...
void CAnimMaterialNode::Animate( SAnimContext &ec )
{
	...
	if(pCh = strstr(pName, ".["))
	{
		...
		MatName[pCh-pName]=0;
		IMaterial * pMat = m_pMovieSystem->GetSystem()->GetI3DEngine()->GetMaterialManager()->FindMaterial( MatName );
		if(!pMat)
			return;
		pCh+=2;
		if(!(*pCh))
	...
	{
		pMtl = m_pMovieSystem->GetSystem()->GetI3DEngine()->GetMaterialManager()->FindMaterial( GetName() );
	}
	...
	IRenderShaderResources *pShaderResources = pMtl->GetShaderItem().m_pShaderResources;
	if (!pShaderResources)
		return;
