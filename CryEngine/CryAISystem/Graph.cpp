																...
																float range, Vec3 * closestValid, bool returnSuspect, const char *requesterName)
{
	...
	IAISystem::ENavigationType navType = GetAISystem()->CheckNavigationType(pos,nGoalBuilding,pGoalArea,navCapMask);
  ...
  if (checkRoadsFirst && navCapMask & IAISystem::NAV_ROAD)
	{
		nodeIndex = GetAISystem()->GetRoadNavRegion()->GetEnclosing(pos, passRadius, startIndex, range, closestValid, returnSuspect, requesterName);
		if (nodeIndex)
			return nodeIndex;
	}
	switch(navType)
	...
	case IAISystem::NAV_FREE_2D:
		nodeIndex = GetAISystem()->GetNavRegion(navType, this)->GetEnclosing(pos, passRadius, startIndex, range, closestValid, returnSuspect, requesterName);
    break;
  default:
...
void CGraph::WriteLine(unsigned nodeIndex)
{
	...
	case IAISystem::NAV_TRIANGULAR:
      ...
      {
        if (!GetAISystem()->m_VertexList.IsIndexValid(pNode->GetTriangularNavData()->vertices[i]))
...
void CGraph::AddIndoorEntrance(int nBuildingID, unsigned nodeIndex, bool bExitOnly)
{
	...
	float radius = 100.0f;
	const SpecialArea * pSA = GetAISystem()->GetSpecialArea(nBuildingID); 
	if (!pSA)
	...
	if (!bAlreadyOutsideLink)
	{
		...
		unsigned outsideNodeIndex = GetAISystem()->GetTriangularNavRegion(this)->GetEnclosing(pNode->GetPos());
		GraphNode *pOutsideNode = GetAISystem()->GetNodeGraph()->GetNode(outsideNodeIndex);
		if (pOutsideNode)
		{
			if (!GetAISystem()->IsPointInForbiddenRegion(pOutsideNode->GetPos()))
...
bool CGraph::ReadNodes( CCryFile &file )
{
	...
	I3DEngine *pEngine = GetAISystem()->m_pSystem->GetI3DEngine();
	NodeDescBuffer::iterator ni;
	int index=0;

	if (/*!GetISystem()->IsEditor() && */iNumber)
	{
		...
		//	if (typeIndex < 0 ||
		//		(/*type != IAISystem::NAV_TRIANGULAR || */type != IAISystem::NAV_UNSET && GetISystem()->IsEditor()))
		//		continue;
		...
		for (ni=m_vNodeDescs.begin();ni != end;++ni,++index)
		{
			...
			// in editor waypoint nodes get added by the editor
			if (type != IAISystem::NAV_TRIANGULAR && type != IAISystem::NAV_UNSET && GetISystem()->IsEditor())
				continue;
			...
			case IAISystem::NAV_WAYPOINT_HUMAN:
				{
					...
					const SpecialArea *sa = GetAISystem()->GetSpecialArea(pNode->GetPos(), areaType);
					if (sa)
					{
						pNode->GetWaypointNavData()->nBuildingID = sa->nBuildingID;
						I3DEngine *p3dEngine=GetAISystem()->m_pSystem->GetI3DEngine();
			...
			mapReadNodes.insert(EntranceMap::iterator::value_type((EntranceMap::key_type)desc.ID, nodeIndex));
	...
	if (ei==mapReadNodes.end())
	{
		...
		GetAISystem()->m_pSystem->GetILog()->UpdateLoadingScreen(" ");
		return false;
	}
	else
	{
	...
	float fStartTime = GetAISystem()->m_pSystem->GetITimer()->GetAsyncCurTime();
	...
	if (!m_vLinkDescs.empty())
	{
		...
		for (LinkDescBuffer::iterator ldbi=m_vLinkDescs.begin();ldbi!=iend;++ldbi)
		{
			...
			if (!ldb.bIsPureTriangularLink && GetISystem()->IsEditor())
				continue;

			ei=mapReadNodes.find((EntranceMap::key_type)ldb.nSourceNode);
			if (ei == mapReadNodes.end())
			{
#if defined __GNUC__
			...
			else
			{
				...
				GraphNode *pNode = GetNodeManager().GetNode(nodeIndex);
				link = mapReadNodes.find((EntranceMap::key_type)ldb.nTargetNode);
	...
	AILogLoading("Finished Reconnecting links in %6.3f sec",
		GetAISystem()->m_pSystem->GetITimer()->GetAsyncCurTime() - fStartTime);
...
unsigned CGraph::CreateNewNode(IAISystem::ENavigationType type, const Vec3 &pos, unsigned ID)
{
	...
	CNavRegion * pNavRegion = GetAISystem()->GetNavRegion(pNode->navType, this);
	if (pNavRegion)
...
void CGraph::DeleteNode(unsigned nodeIndex)
{
	...
	if (pNode->Release())
	{
		...
		if (GetAISystem()->GetWaypointHumanNavRegion())
			GetAISystem()->GetWaypointHumanNavRegion()->ResetUpdateStatus();
	...
	while (!openList.empty())
	{
		...
		for (unsigned link = pNode->firstLinkIndex; link; link = linkManager.GetNextLink(link))
		{
			...
			if (pRequester && pNode->navType == IAISystem::NAV_SMARTOBJECT && pNext->navType == IAISystem::NAV_SMARTOBJECT)
			{
				...
				float resFactor = GetAISystem()->GetSmartObjectLinkCostFactor(nodes, pRequester);
				if ( resFactor < 0 )
					continue;
			}
			...
			if (totalDist <= maxDist)
			{
				...
				if (pNext->mark == 1)
				{
					for (OpenListMap::iterator it = openList.begin(), end = openList.end(); it != end; ++it)
					{
						if (it->second == pNext)
						{
							found = it;
...
/*
	static MapConstNodesDistance openList;
	openList.clear();
	openList[pStartNode] = 0.0f;

	result[pStartNode] = 0.0f;

	while (!openList.empty())
	{
		MapConstNodesDistance::iterator front = openList.begin();
		const GraphNode *pNode = front->first;
		float curDist = front->second;
		openList.erase(front);
	...
	for (unsigned int i=0;i<pOne->GetTriangularNavData()->vertices.size();i++)
		...
		for (unsigned link = pOne->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
		{
			if (GetNodeManager().GetNode(GetLinkManager().GetNextNode(link)) == pTwo)
			{
				...
				Vec3 edge = GetAISystem()->m_VertexList.GetVertex(pOne->GetTriangularNavData()->vertices[iOneEdge1]).vPos 
					- GetAISystem()->m_VertexList.GetVertex(pOne->GetTriangularNavData()->vertices[iOneEdge2]).vPos;
				GetLinkManager().GetEdgeCenter(link) = GetAISystem()->m_VertexList.GetVertex(pOne->GetTriangularNavData()->vertices[iOneEdge1]).vPos - 0.5f * edge;
		...
		for (unsigned link = pTwo->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
		{
			if (GetNodeManager().GetNode(GetLinkManager().GetNextNode(link)) == pOne)
			{
				...
				Vec3 edge = GetAISystem()->m_VertexList.GetVertex(pTwo->GetTriangularNavData()->vertices[iTwoEdge1]).vPos 
					- GetAISystem()->m_VertexList.GetVertex(pTwo->GetTriangularNavData()->vertices[iTwoEdge2]).vPos;

				GetLinkManager().GetEdgeCenter(link) = GetAISystem()->m_VertexList.GetVertex(pTwo->GetTriangularNavData()->vertices[iTwoEdge1]).vPos - 0.5f * edge;
				AIAssert(GetLinkManager().GetEdgeCenter(link).GetLength() > 0.0f);
				break;
			}
		}
...
void CGraph::FillGraphNodeData(unsigned nodeIndex)
{
	...
	CAISystem *pAI = GetAISystem();
	I3DEngine *pEngine = pAI->m_pSystem->GetI3DEngine();
	...
	for (ObstacleIndexVector::const_iterator oi = pTriNavData->vertices.begin(); oi != oiEnd ; ++oi)
	{
		...
		Vec3 pt = pAI->m_VertexList.GetVertex(*oi).vPos;
		Vec3 ptNext = pAI->m_VertexList.GetVertex(*oiNext).vPos;
		Vec3 ptNextNext = pAI->m_VertexList.GetVertex(*oiNextNext).vPos;
	...
	if (totalLen > 0.0f)
		...
		pos = pAI->m_VertexList.GetVertex(pTriNavData->vertices.front()).vPos;
	else
		pos.zero();
...
{
	unsigned firstIndex = GetEnclosing(vFirst,IAISystem::NAVMASK_SURFACE);
	unsigned secondIndex = GetEnclosing(vSecond,IAISystem::NAVMASK_SURFACE);
	unsigned thirdIndex = GetEnclosing(vThird,IAISystem::NAVMASK_SURFACE);

	GraphNode* pFirst = GetNodeManager().GetNode(firstIndex);
	if (pFirst->GetLinkCount(GetLinkManager())>2)
	{
		for (unsigned link = pFirst->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
			if ((GetLinkManager().GetNextNode(link) != secondIndex) && (GetLinkManager().GetNextNode(link)!=thirdIndex))
				return GetNodeManager().GetNode(GetLinkManager().GetNextNode(link));
	}
	else
	{
		for (unsigned link = pFirst->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
			if ( GetLinkManager().GetNextNode(link) != secondIndex )
				return GetNodeManager().GetNode(GetLinkManager().GetNextNode(link));
	}
...
void CGraph::DisableInSphere(const Vec3 &pos,float fRadius)
{
	...
	ListNodeIds::iterator li = m_lstNodesInsideSphere.begin(),liend = m_lstNodesInsideSphere.end();
	for (;li!=liend;++li)
	{
		GraphNode *pNode = GetNodeManager().GetNode(*li);
		if (pNode->navType == IAISystem::NAV_TRIANGULAR) 
			continue;

		for (unsigned link = pNode->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
		{
			GetLinkManager().ModifyRadius(link, -1.0f);	// forbid this link

			GraphNode *pNextNode = GetNodeManager().GetNode(GetLinkManager().GetNextNode(link));
			for (unsigned incomingLink = pNextNode->firstLinkIndex; incomingLink; incomingLink = GetLinkManager().GetNextLink(incomingLink))
			{
				if (GetNodeManager().GetNode(GetLinkManager().GetNextNode(incomingLink)) == pNode)
				{
					GetLinkManager().ModifyRadius(incomingLink, -1.0f);
					break;
				}
			}
	...
	for (;li!=liend;++li)
	{
		...
		for (unsigned link = pNode->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
		{
			...
			for (unsigned incomingLink = pNextNode->firstLinkIndex; incomingLink; incomingLink = GetLinkManager().GetNextLink(incomingLink))
			{
				if (GetNodeManager().GetNode(GetLinkManager().GetNextNode(incomingLink)) == pNode)
				{
					GetLinkManager().RestoreLink(incomingLink);
					break;
				}
			}
		}
...
{
	unsigned nodeIndex = GetEnclosing(pos,IAISystem::NAVMASK_SURFACE);
	SMarkClearer markClearer(this);
	m_lstNodesInsideSphere.clear();
	if (!nodeIndex)
	{
		AIWarning("CGraph::GetNodesInSphere Could not find node enclosing (%5.2f, %5.2f, %5.2f)", 
			pos.x, pos.y, pos.z);
		return 0;
	}

	if (GetNodeManager().GetNode(nodeIndex)->navType == IAISystem::NAV_TRIANGULAR)
		return 0;


	m_lstNodesInsideSphere.push_front(nodeIndex);
	while(GetNodeManager().GetNode(m_lstNodesInsideSphere.front())->mark == false)
	{

		ListNodeIds::iterator li = m_lstNodesInsideSphere.begin(),liend = m_lstNodesInsideSphere.end();
		for (;li!=liend;)
		{
			ListNodeIds::iterator linext = li;
			++linext;
			if (linext!=liend)
			{
				if (GetNodeManager().GetNode(*linext)->mark)
					break;
			}
			++li;
		}
		...
		for (unsigned link = GetNodeManager().GetNode(nodeIndex)->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
		{
			unsigned nextNodeIndex = GetLinkManager().GetNextNode(link);
			if (GetNodeManager().GetNode(nextNodeIndex)->mark)
				continue;
			if ( (GetNodeManager().GetNode(nextNodeIndex)->GetPos()-pos).GetLength() < fRadius )
			{
				m_lstNodesInsideSphere.push_front(nextNodeIndex);
			}
		}
	}
...
// BreakNodes
//====================================================================
void CGraph::BreakNodes(int nBuildingID)
{
	EntranceMap::iterator ei = m_mapRemovables.find(nBuildingID);
	if (ei!=m_mapRemovables.end())
	{
		while ( (ei!=m_mapRemovables.end()) && (ei->first == nBuildingID))
		{
			// for all removable nodes make all the links unpassible
			GraphNode* pDisconnected=GetNodeManager().GetNode(ei->second);
			for (unsigned link = pDisconnected->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
			{
				// not RADIUS_FOR_BROKEN_LINKS since that is just used for breakable geometry
				GetLinkManager().ModifyRadius(link, -1.0f);
			}
			++ei;
		}
	}
...
// RestoreBrokenLinks
//====================================================================
void CGraph::RestoreBrokenLinks()
{
	...
	while (GraphNode* node = GetNodeManager().GetNode(it.Increment()))
	{
		...
		{
			if (fabsf(GetLinkManager().GetRadius(link) - RADIUS_FOR_BROKEN_LINKS) < 0.01f)
				GetLinkManager().RestoreLink(link);
		}
	}
}
...
//====================================================================
// DisableNavigationInBrokenRegions
//====================================================================
void CGraph::DisableNavigationInBrokenRegions(const CRegions & regions)
{
	FUNCTION_PROFILER( GetISystem(), PROFILE_AI );

	// if AI isn't loaded
	if (!m_pSafeFirst || !m_pSafeFirst->firstLinkIndex)
		return;

	CAllNodesContainer::Iterator it(m_allNodes, IAISystem::NAV_TRIANGULAR);
	while (GraphNode* node = GetNodeManager().GetNode(it.Increment()))
	{
		bool nodeBroken = false;
		if (node->GetTriangularNavData()->vertices.size() == 3)
		{
			const Vec3 & v0 = GetAISystem()->m_VertexList.GetVertex(node->GetTriangularNavData()->vertices[0]).vPos;
			const Vec3 & v1 = GetAISystem()->m_VertexList.GetVertex(node->GetTriangularNavData()->vertices[1]).vPos;
			const Vec3 & v2 = GetAISystem()->m_VertexList.GetVertex(node->GetTriangularNavData()->vertices[2]).vPos;
			nodeBroken = DoesTriangleOverlapRegions(v0, v1, v2, regions);
		}
		else
		{
			nodeBroken = IsPointInRegions(node->GetPos(), regions);
		}

		for (unsigned link = node->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
		{
			GraphNode * otherNode = GetNodeManager().GetNode(GetLinkManager().GetNextNode(link));

			if (nodeBroken)
			{
				for (unsigned otherLink = otherNode->firstLinkIndex; otherLink; otherLink = GetLinkManager().GetNextLink(otherLink))
				{
					if (GetNodeManager().GetNode(GetLinkManager().GetNextNode(otherLink)) == node)
					{
						if (GetLinkManager().GetRadius(otherLink) > 0.0f)
						{
							// only disable the link if the other is not broken, to allow some walking
							bool otherNodeBroken = IsPointInRegions(otherNode->GetPos(), regions);

							if (!otherNodeBroken)
								GetLinkManager().ModifyRadius(otherLink, RADIUS_FOR_BROKEN_LINKS);
						}
					}
				}
			}
...
// CheckForEmpty
//====================================================================
bool CGraph::CheckForEmpty(unsigned navTypeMask) const
{
	unsigned count = 0;
	CAllNodesContainer::Iterator it(m_allNodes, navTypeMask);
...
// GetBestPosition
//====================================================================
Vec3 CGraph::GetBestPosition(const CHeuristic& heuristic, float maxCost, 
														 const Vec3& startPos, const Vec3& endPos, 
														 unsigned startHintIndex,
														 IAISystem::tNavCapMask navCapMask)
{
	unsigned startNodeIndex = GetEnclosing(startPos, navCapMask, 0.0f, startHintIndex);
	if (!startNodeIndex)
		return startPos;
	unsigned endNodeIndex = GetEnclosing(endPos, navCapMask, 0.0f, startNodeIndex);

	GraphNode* startNode = GetNodeManager().GetNode(startNodeIndex);
	GraphNode* endNode = GetNodeManager().GetNode(endNodeIndex);
	if (endNodeIndex && endNode->navType == IAISystem::NAV_TRIANGULAR && startNode->navType == IAISystem::NAV_TRIANGULAR)
	{
		...
		float cost = GetAISystem()->AttemptStraightPath(straightPath, pfr, startPos, &heuristic, 
	...
	typedef std::pair<float, GraphNode*> CostNodePair;
	std::map<GraphNode*, float> checkedNodes;
	...
	while (!pendingNodes.empty())
	{
		...
		GraphNode* pNode = currentPair.second;

		bool needToCheckThisNode = false;
		std::map<GraphNode*, float>::iterator it = checkedNodes.find(pNode);
		if (it == checkedNodes.end() || it->second > cost)
		{
			needToCheckThisNode = true;
			checkedNodes[pNode] = cost;
		}
		...
		if (needToCheckThisNode)
		{
			for (unsigned link = pNode->firstLinkIndex; link; link = GetLinkManager().GetNextLink(link))
			{
				GraphNode* pNext = GetNodeManager().GetNode(GetLinkManager().GetNextNode(link));

				if (!(pNext->navType & navCapMask))
					continue;

				float costToNext = heuristic.CalculateCost(this, *pNode, link, *pNext, NavigationBlockers());
				if (costToNext <= 0.0f)
					continue;
	...
	GraphNode fakeNode(IAISystem::NAV_UNSET, endPos, 1);
	for (std::map<GraphNode*, float>::iterator it = checkedNodes.begin() ; it != checkedNodes.end() ; ++it)
	{
		GraphNode* pNode = it->first;
	...
	return bestNode ? bestNode->GetPos() : startPos;
}
...
void CGraph::Serialize( TSerialize ser, CObjectTracker& objectTracker, const char* graphName )
{
  ...
  ser.BeginGroup( graphName );
  {
    ...
    ser.BeginGroup("GraphPointers");
    {
      ...
      SerializeNodePointerContainer(ser, "m_lstNodesInsideSphere", m_lstNodesInsideSphere, &graphNodeIDs);
    }
    ser.EndGroup();
    ...
    ser.BeginGroup("Link_Modifiers");
    {
      ...
      else
      {
        ...
        ser.Value("numModifiers", numModifiers);
        for (std::vector<SLinkModifier>::iterator it = linkModifiers.begin() ; it != linkModifiers.end() ; ++it)
        {
          SLinkModifier& mod = *it;
          mod.Serialize(ser, this);
        }
      }
    }
...
{
	objectTracker.SerializeObjectPointer(ser, "m_pAnchor", m_pAnchor, false);
...
unsigned CGraph::GetGraphLink(const GraphNode *pNode, unsigned iLink) {unsigned link; for (link = pNode->firstLinkIndex; link && iLink; link = GetLinkManager().GetNextLink(link), --iLink); return link;}
