...
struct Vtx
{
	...
	Vtx(real x = 0.0f, real y = 0.0f, real z = 0.0f) : x(x), y(y), z(z) {bCollidable = true;}
	Vtx(const Vec3r& v) : x(v.x), y(v.y), z(v.z) {bCollidable = true;}
...
public:
	...
	int AddVertex(real x, real y, real z, bool bCollidable = true);
