...
CImageJpgFile::CImageJpgFile (byte* ptr, long filesize) : CImageFile ()
{
  ...
  /* And we're done! */
#endif	//#ifndef USE_IJL
}

#endif // WIN64

#if defined(WIN64) || defined (PS3) || defined(LINUX)
	extern "C"
	{
		#include "jpeg6/jpeglib.h"
#if defined(WIN64) || defined(PS3)
		#include "Jmemsrc.c" // include buffer source input code
#else
		extern void my_jpeg_memory_src (j_decompress_ptr cinfo, char * inbfr, int len);
#endif
	}
#include <setjmp.h>
...
bool WriteJPG( byte *dat, int wdt, int hgt, const char *name, int bpp )
{
...
#if !defined(WIN64) && !defined(NULL_RENDERER) && !defined(_XBOX) && !defined(PS3) && !defined(LINUX)
#if USE_IJL
	if(bpp!=24 && bpp!=32)
		return false;
  ...
  if( ijlInit( &image ) != IJL_OK )
	...
	if(bpp==32)
	{
		data = new byte [wdt*hgt*3];
		for (int i=0; i<wdt*hgt; i++)
		{
			data[i*3+0] = dat[i*4+2];
			data[i*3+1] = dat[i*4+1];
			data[i*3+2] = dat[i*4+0];
		}
	}
  ...
  bOk = ijlWrite( &image, IJL_JFILE_WRITEWHOLEIMAGE ) == IJL_OK;
