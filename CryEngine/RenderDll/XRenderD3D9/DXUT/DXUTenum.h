...
UINT    WINAPI DXUTGetD3D9ColorBits( D3DFORMAT fmt );
...
public:
    ...
    void GetResolutionMinMax( UINT& nMinWidth, UINT& nMinHeight, UINT& nMaxWidth, UINT& nMaxHeight );  
...
private:
    friend CD3D9Enumeration* WINAPI DXUTGetD3D9Enumeration( bool bEnumerate ); 
...
private:
    friend CD3D10Enumeration* WINAPI DXUTGetD3D10Enumeration( bool bEnumerate );
    ...
    void BuildMultiSampleQualityList( DXGI_FORMAT fmt, ID3D10Device *pd3dDevice, CD3D10EnumDeviceInfo* pDeviceInfo );
    void ClearAdapterInfoList();
};

CD3D10Enumeration* WINAPI DXUTGetD3D10Enumeration( bool bForceEnumerate = false );
