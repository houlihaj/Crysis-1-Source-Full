...
namespace ATG
{
	...
	// FourCC definitions
	//--------------------------------------------------------------------------------------
	const DWORD FOURCC_RIFF   = 'RIFF';
	const DWORD FOURCC_WAVE   = 'WAVE';
	const DWORD FOURCC_FORMAT = 'fmt ';
	const DWORD FOURCC_DATA   = 'data';
	const DWORD FOURCC_WSMP   = 'wsmp';
	const DWORD FOURCC_SMPL   = 'lsmp';
	...
	HRESULT WaveFile::GetFormat( WAVEFORMATEXTENSIBLE* pwfxFormat ) const
	{
		...
		HRESULT hr = m_FormatChunk.ReadData( 0, pwfxFormat, dwValidSize );
		if( FAILED(hr) )
			return hr;
