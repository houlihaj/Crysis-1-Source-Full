...
bool CMusicPattern::Open( const char* pszFilename )
{
	...
	if( false == m_pMusicSystem->StreamOGG() || false != bWaveFallback )
	{
		...
		{
			m_pMusicSystem->LogMsg(1, "[Error] <Music> Pattern: OGG file %s not found... trying .snd", strFilename.c_str() );
		}
		...
		if( 0 != pFile )
		{
			...
			if( WAVE_FORMAT_ID_ADPCM == header.wOne_0 )
			{
				...
				m_pMusicSystem->LogMsg(2, "<Music> Pattern: Initialized ADPCM Decoder for %s", strFilename.c_str() );
			}
			else
			if( WAVE_FORMAT_ID_PCM == header.wOne_0 )
			{
				m_pDecoder = new CPCMDecoder( m_pMusicSystem );	
				m_pMusicSystem->LogMsg(2, "<Music> Pattern: Initialized PCM Decoder for %s", strFilename.c_str() );
			}
			else
			{
				m_pMusicSystem->LogMsg(1, "[Warning] <Music> Pattern: AD(PCM) expected... failed to initialize %s", strFilename.c_str() );
				return( false );
			}
			pPak->FClose( pFile );
		}
