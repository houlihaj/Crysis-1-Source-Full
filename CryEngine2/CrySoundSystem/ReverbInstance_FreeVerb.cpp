...
void CReverbInstance_FreeVerb::FmodErrorOutput(const char * sDescription, IMiniLog::ELogType LogType)
{
	switch(LogType) 
	{
...
bool CReverbInstance_FreeVerb::Activate()
{
	...
	if (m_pCSEX && !m_DSPReverb)
	{
		...
		{
			FmodErrorOutput("create freeverb reverb failed! ", IMiniLog::eError);
			return false;
		}
		...
		if (m_DSPReverb)
		{
			...
			if (m_ExResult != FMOD_OK)
			{
				FmodErrorOutput("add freeverb to DSP chain failed! ", IMiniLog::eError);
				return false;
			}
			...
			if (m_ExResult != FMOD_OK)
			{
				FmodErrorOutput("activate freeverb failed! ", IMiniLog::eError);
				return false;
			}
