...
void CPlatformSoundFmodEx400::FmodErrorOutput(const char * sDescription, IMiniLog::ELogType LogType)
{
...
bool CPlatformSoundFmodEx400::Set3DPosition(Vec3* pvPosition, Vec3* pvVelocity, Vec3* pvOrientation)
{
	...
	if (m_pExChannel)
	{
		...
		if (IS_FMODERROR)
		{
			FmodErrorOutput("get channel mode failed! ", IMiniLog::eWarning);
			return false;
		}
		...
		if (IS_FMODERROR)
		{
			FmodErrorOutput("set channel mode failed! ", IMiniLog::eWarning);
			return false;
		}
		...
		if (IS_FMODERROR)
		{
			FmodErrorOutput("set 3d channel position failed! ", IMiniLog::eWarning);
			return false;
		}
	}
	return (true);
	...
	case pspFREQUENCY:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel frequency failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspVOLUME:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel volume failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspISPLAYING:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel isplaying failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspPAUSEMODE:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel paused failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspSPEAKERPAN:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get pan level failed! ", IMiniLog::eWarning);
				return false;
			}
			pParam->SetValue(fTemp);
			break;
	...
	case pspSPEAKER3DPAN:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get 3d pan level failed! ", IMiniLog::eWarning);
				return false;
			}
			pParam->SetValue(fTemp);
			break;
	...
	case pspSAMPLEPOSITION:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel sample position failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspTIMEPOSITION:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel time position failed! ", IMiniLog::eWarning);
				return false;
			}
...
//		TIMEPOSITION
	...
	case psp3DPOSITION:
		...
		if (Mode & FMOD_3D)
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel 3d position failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case psp3DVELOCITY:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel 3d velocity failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspPRIORITY:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel priority failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspAMPLITUDE:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel audibility failed! ", IMiniLog::eWarning);
				return false;
			}
...
bool CPlatformSoundFmodEx400::SetParamByType(enumPlatformSoundParamSemantics eSemantics, ptParam* pParam)
{
	...
	case pspFREQUENCY:
		// TODO Check Pitch and stuff
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel frequency failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspVOLUME:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel volume failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspLOOPMODE:
		{
			...
			else
			{
				...
				if (IS_FMODERROR)
				{
					FmodErrorOutput("set channel paused failed! ", IMiniLog::eWarning);
					return false;
				}
	...
	case pspSPEAKERPAN:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set pan level failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspSPEAKER3DPAN:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set 3d pan level failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspSAMPLEPOSITION:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel sample position failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspTIMEPOSITION:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel time position failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case psp3DPOSITION:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel 3d position failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case psp3DVELOCITY:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel 3d velocity failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspMINRADIUS:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel min/max radius failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspPRIORITY:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel priority failed! ", IMiniLog::eWarning);
				return false;
			}
	...
	case pspREVERBWET:
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("get channel reverb props failed! ", IMiniLog::eWarning);
				return false;
			}
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("set channel reverb props failed! ", IMiniLog::eWarning);
				return false;
			}
...
bool CPlatformSoundFmodEx400::StopSound()
{
	...
	if (m_pExChannel)
	{
		...
		if (bPlaying)
		{
			...
			if (IS_FMODERROR)
			{
				FmodErrorOutput("stop channel failed! ", IMiniLog::eWarning);
				return false;
			}
		}
	}
...
bool CPlatformSoundFmodEx400::CreateSound(tAssetHandle AssetHandle, SSoundSettings SoundSettings)
{
	...
	if (SoundBuf && !SoundBuf->LoadFailure())
	{
		...
		case btSTREAM:
			...
			{
				FmodErrorOutput("create stream channel failed! ", IMiniLog::eWarning);
				return false;
			}
		...
		case btSAMPLE:
			...
			if (IS_FMODERROR || m_pExChannel==NULL)
			{
				FmodErrorOutput("create sample channel failed! ", IMiniLog::eWarning);
				return false;
			}
