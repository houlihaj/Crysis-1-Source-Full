...
static const	char		*memoryLeakLogFile     = "memleaks1.log";
static		void		doCleanupLogOnFirstRun();
static  unsigned int	loggedAllocations = 0;
static  unsigned int	loggedAllocationsTotal = 0;
//static bool staticLogAllocations = false;

void setLogAllocations(bool v)
{
	alwaysLogAll = v;
	//if (!v)
	//	loggedAllocationsTotal += loggedAllocations;
...
static	void	dumpLeakReport()
{
	...
	FILE	*fp = fopen(memoryLeakLogFile, "ab");
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		/*
		if (currentAllocationCount == 51694)
		{
		assert(false);
		}
		if (currentAllocationCount == 5223)
		{
		assert(false);
		}
		if (currentAllocationCount == 22114)
		{
		assert(false);
		}
		if (currentAllocationCount == 51606)
		{
		assert(false);
		}
		if (currentAllocationCount == 23936)
		{
		assert(false);
		}
		*/
		...
		loggedAllocations += reportedSize;

		if (alwaysLogAll) 
		{
			loggedAllocationsTotal += reportedSize;
			log("[+] %d %s of size 0x%X(%d) by %s. Total %d. TotalTotal %u", currentAllocationCount, allocationTypes[allocationType], reportedSize, reportedSize, ownerString(sourceFile, sourceLine, sourceFunc), loggedAllocations, loggedAllocationsTotal);
		}
		...
		int len = sizeof(au->sourceFile) - 1;
		if (len > 100)
			len = 100;
		if (sourceFile) strncpy(au->sourceFile, sourceFileStripper(sourceFile), len);
		else		strcpy (au->sourceFile, "??");
		len = sizeof(au->sourceFunc) - 1;
		if (len > 100)
			len = 100;

		if (sourceFunc) strncpy(au->sourceFunc, sourceFunc, len);
