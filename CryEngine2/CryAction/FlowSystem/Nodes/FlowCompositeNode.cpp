...
void CFlowCompositeNodeFactory::GetMemoryStatistics(ICrySizer * s)
{
	SIZER_SUBCOMPONENT_NAME(s, "CFlowCompositeNodeFactory");
	s->Add(*this);
	s->AddContainer(m_stringPool);
	s->AddContainer(m_inputsInt);
	s->AddContainer(m_inputsExt);
	s->AddContainer(m_outputsInt);
	s->AddContainer(m_outputsExt);
	for (std::vector<string>::iterator iter = m_stringPool.begin(); iter != m_stringPool.end(); ++iter)
		s->Add(*iter);
	for (std::vector<SInputPortConfig>::iterator iter = m_inputsInt.begin(); iter != m_inputsInt.end(); ++iter)
		iter->defaultData.GetMemoryStatistics(s);
	for (std::vector<SInputPortConfig>::iterator iter = m_inputsExt.begin(); iter != m_inputsExt.end(); ++iter)
		iter->defaultData.GetMemoryStatistics(s);
}

//////////////////////////////////////////////////////////////////////////
class CFlowNode_Composite_Input : public CFlowBaseNode
{
public:
...
//////////////////////////////////////////////////////////////////////////
class CFlowNode_Composite_Output : public CFlowBaseNode
{
public:
