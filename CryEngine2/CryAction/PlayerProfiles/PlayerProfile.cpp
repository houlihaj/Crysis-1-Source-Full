...
bool CPlayerProfile::SerializeXML(CPlayerProfileManager::IProfileXMLSerializer* pSerializer)
{
	if (pSerializer->IsLoading())
	{
		...
		XmlNodeRef attributesNode = pSerializer->GetSection(ATTRIBUTES_TAG);
		if (attributesNode)
		{
		...
		if (m_bIsPreview == false)
		{
			...
			XmlNodeRef actionMaps = pSerializer->GetSection(ACTIONMAPS_TAG);
			if (actionMaps)
			{
				...
				{
					pAM->LoadFromXML(actionMaps, true); // check version and don't load if outdated
				}
			}
			else
			{
	...
	else
	{
		...
		{
			// serialize attributes
			XmlNodeRef attributesNode = pSerializer->AddSection(ATTRIBUTES_TAG);
			...
			// serialize action maps
			IActionMapManager* pAM = CCryAction::GetCryAction()->GetIActionMapManager();
			pAM->SaveToXML(pSerializer->AddSection(ACTIONMAPS_TAG));
			return ok;
		}
	}
	return true;
...
void CPlayerProfile::GetMemoryStatistics(ICrySizer * s)
{
	s->Add(*this);
	s->Add(m_name);
	s->Add(m_userId);
	for (TAttributeMap::iterator iter = m_attributeMap.begin(); iter != m_attributeMap.end(); ++iter)
	{
		s->Add(iter->first);
