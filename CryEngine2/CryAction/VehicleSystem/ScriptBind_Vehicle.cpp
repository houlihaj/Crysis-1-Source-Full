...
#define SCRIPT_REG_CLASSNAME &CScriptBind_Vehicle::
	...
	SCRIPT_REG_TEMPLFUNC(IsRepairableDamage, "");
	...
	SCRIPT_REG_TEMPLFUNC(SetMovement, "movement, params");
  ...
  SCRIPT_REG_TEMPLFUNC(GetSelfCollisionMult, "velocity, normal, partId, colliderId");
	SCRIPT_REG_TEMPLFUNC(GetPlayerCollisionMult, "");
...
CVehicle *CScriptBind_Vehicle::GetVehicle(IFunctionHandler *pH)
{
  ...
  if (pH->GetSelf(table))
  {
    ...
    { 
      return (CVehicle*)m_pVehicleSystem->GetVehicle(handle.n);
    }
  }
...
int CScriptBind_Vehicle::SetPlayerToSit(IFunctionHandler *pH, ScriptHandle playerId, int flags)
{
  ...
  if (!vehicle)
	...
	IEntity* playerEntity = gEnv->pEntitySystem->GetEntity(playerId.n);
	if (playerEntity == NULL)
		return pH->EndFunction();

	if (flags == 1)
	{
		gEnv->pGame->GetIGameFramework()->GetGameObject(playerId.n)->OnAction("UseVehicle", (int)vehicleEntity->GetId(), 0.0f);
	}
	else
	{
		gEnv->pGame->GetIGameFramework()->GetGameObject(playerId.n)->OnAction("UseVehicle", 0, 0.0f);
	}
...
int CScriptBind_Vehicle::GetRepairableDamage(IFunctionHandler *pH)
{
	...
	if (pVehicle)
	{
		// HUD displays the hull damage now, so we should do the same here.
		...
		return pH->EndFunction(hullDamage);
	}

	return pH->EndFunction(0.0f);
}

int CScriptBind_Vehicle::IsRepairableDamage(IFunctionHandler* pH)
{
	CVehicle *pVehicle = GetVehicle(pH);

	if (pVehicle)
	{
		return pH->EndFunction(pVehicle->GetDamageRatio() > 0.0f);
	}
...
int CScriptBind_Vehicle::MultiplyWithEntityLocalTM(IFunctionHandler *pH, ScriptHandle entityHandle, Vec3 pos)
{
	...
	IEntity* e = gEnv->pEntitySystem->GetEntity(entityHandle.n);
	if (!e)
		return pH->EndFunction();
...
int CScriptBind_Vehicle::UpdateVehicleAnimation(IFunctionHandler *pH, ScriptHandle entityHandle, int slot)
{
	...
	IEntity* part = gEnv->pEntitySystem->GetEntity(entityHandle.n);
	if (!part)
		return pH->EndFunction();
	...
	if (charInstance)
	{
		...
		charInstance->SkeletonPostProcess(renderLocation, renderLocation, 0, (part->GetWorldTM().GetTranslation() - gEnv->pSystem->GetViewCamera().GetPosition()).GetLength(), 0);
	}
	return pH->EndFunction();
}
...
int CScriptBind_Vehicle::RefreshPhysicsGeometry(IFunctionHandler *pH, ScriptHandle partHandle, int slot, int physIndex)
{
	...
	IEntity* part = gEnv->pEntitySystem->GetEntity(partHandle.n);
	if (!part)
		return pH->EndFunction();
...
//------------------------------------------------------------------------
int CScriptBind_Vehicle::SetMovement(IFunctionHandler *pH, char* movementClass, SmartScriptTable paramTable)
{
	if (CVehicle* pVehicle = GetVehicle(pH))
	{
		pVehicle->SetMovement(movementClass, paramTable);
	}
...
int CScriptBind_Vehicle::OnHit(IFunctionHandler* pH, ScriptHandle targetId, ScriptHandle shooterId, float damage, Vec3 position, float radius, char* pHitClass, bool explosion)
{
	if (CVehicle* pVehicle = GetVehicle(pH))
	{
		pVehicle->OnHit(targetId.n, shooterId.n, damage, position, radius, pHitClass, explosion);
	}
	return pH->EndFunction();
}
...
int CScriptBind_Vehicle::ProcessPassengerDamage(IFunctionHandler* pH, ScriptHandle passengerHandle, float actorHealth, float damage, const char* pDamageClass, bool explosion)
{
	...
	if (CVehicleSeat* pSeat = (CVehicleSeat*)pVehicle->GetSeatForPassenger(passengerHandle.n))
	{
...
int CScriptBind_Vehicle::IsUsable(IFunctionHandler* pH, ScriptHandle userHandle)
{
	...
	int ret = pVehicle->IsUsable(userHandle.n);
...
int CScriptBind_Vehicle::OnUsed(IFunctionHandler* pH, ScriptHandle userHandle, int index)
{
	...
	int ret = pVehicle->OnUsed(userHandle.n, index);
	return pH->EndFunction(ret);
}
...
int CScriptBind_Vehicle::EnterVehicle(IFunctionHandler* pH, ScriptHandle actorHandle, int seatId, bool isAnimationEnabled)
{
	...
	if (pSeat)
		ret = pSeat->Enter(actorHandle.n, isAnimationEnabled);
...
int CScriptBind_Vehicle::ChangeSeat(IFunctionHandler* pH, ScriptHandle actorHandle, int seatId, bool isAnimationEnabled)
{
	...
	bool ret = true;
	pVehicle->ChangeSeat(actorHandle.n, seatId, 0 );
...
int CScriptBind_Vehicle::ExitVehicle(IFunctionHandler* pH, ScriptHandle actorHandle)
{
	...
	if (IVehicleSeat* pSeat = pVehicle->GetSeatForPassenger(actorHandle.n))
	{
...
int CScriptBind_Vehicle::GetSelfCollisionMult(IFunctionHandler* pH, Vec3 velocity, Vec3 normal, int partId, ScriptHandle colliderId)
{
  if (CVehicle* pVehicle = GetVehicle(pH))
  {     
    return pH->EndFunction(pVehicle->GetSelfCollisionMult(velocity, normal, partId, colliderId.n));    
  }
...
//------------------------------------------------------------------------
int CScriptBind_Vehicle::GetPlayerCollisionMult(IFunctionHandler* pH)
{
	if(CVehicle* pVehicle = GetVehicle(pH))
	{
		return pH->EndFunction(pVehicle->GetPlayerCollisionMult());
	}
