...
//------------------------------------------------------------------------
bool CVehiclePartMassBox::Init(IVehicle* pVehicle, const SmartScriptTable &table, IVehiclePart* parent, CVehicle::SPartInitInfo& initInfo)
{
  ...
  if (table->GetValue("MassBox", subTable))
  {
    subTable->GetValue("size", m_size);
    subTable->GetValue("drivingOffset", m_drivingOffset);    
  }
...
//------------------------------------------------------------------------
void CVehiclePartMassBox::Serialize(TSerialize ser, unsigned aspects)
{	
