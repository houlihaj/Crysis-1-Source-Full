...
//------------------------------------------------------------------------
bool CVehicleSeatActionSound::Init(IVehicle* pVehicle, TVehicleSeatId seatId, const SmartScriptTable &table)
{
	m_pVehicle = pVehicle;
	m_seatId = seatId;

	SmartScriptTable soundTable;
	if (!table->GetValue("Sound", soundTable))
		return false;

	char* pSoundName;
	if (!soundTable->GetValue("sound", pSoundName))
		return false;
	...
	char* pHelperName = 0;
	if (soundTable->GetValue("helper", pHelperName))
