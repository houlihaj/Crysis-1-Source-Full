													 ...
													 const Vec3r &center, int flags, float tolerance, primitive *&pprim)
{
	...
	if (flags & mesh_approx_cylinder) {
		...
		for(iz=0;iz<3;iz++) {
			...
			locerror = max(locerror, fabs_tpl((area[1]-r[iz]*r[iz]*g_PI*2)*(rinv*rinv*((real)0.5/g_PI))));
			error_max[iz] = max(error_max[iz], locerror);
	...
	if (flags & mesh_approx_capsule) {
		...
		for(iz=0;iz<3;iz++) {
			...
			// additionally check if object area is close to that of the cylinder
			locerror = fabs_tpl((area[0]-r[iz]*h[iz]*g_PI*4)*(rinv*hinv*((real)0.5/g_PI)));
			locerror = max(locerror, fabs_tpl((area[1]-r[iz]*r[iz]*g_PI*4)*(rinv*rinv*((real)0.25/g_PI))));
			error_max[iz] = max(error_max[iz], locerror);
			error_avg[iz] = error_avg[iz]*(real)0.7+locerror*(real)0.3;
			bBest = isneg(error_avg[iz]-error_avg[ibest]);
			ibest = ibest&~-bBest | iz&-bBest;
...
int BreakPolygon(vector2df *ptSrc,int nPt, int nCellx,int nCelly, int maxPatchTris, vector2df *&ptout,int *&nPtOut, float jointhresh,int seed)
{
	...
	for(ix=0;ix<sz.x;ix++) for(iy=0;iy<sz.y;iy++)	// generate jittered grid points
		pt[ix+iy*sz.x].set(
			(float2int((ix*step.x+physics_frand(step.x*0.45f)-snap*0.5f)*rsnap)+0.5f)*snap, 
			(float2int((iy*step.y+physics_frand(step.y*0.45f)-snap*0.5f)*rsnap)+0.5f)*snap);
	...
	for(i=nCells-1;i>=0;i--) { 
		pCellDiv[i] = physics_rand()&1;	// randomly choose the way each cell is split
	...
	do {
		nPatchTris = float2int(physics_frand(maxPatchTris)-0.5f);	// request random number of triangles for this patch
...
static ComputeBitCount computeBitCount;
