...
int CBoxGeom::PointInsideStatus(const Vec3 &pt)
{
	...
	return isneg(max(max(fabsf(ptloc.x)-m_box.size.x,fabsf(ptloc.y)-m_box.size.y),fabsf(ptloc.z)-m_box.size.z));
}
		...
		for(iz=0;iz<3;iz++) {
			...
			{//for(iedge=0;iedge<4;iedge++) {
				...
				if (inrange(t0, (float)0,n2) & isneg(max(fabs_tpl(t1)-n2*size[iz],fabs_tpl((ptbox-ptloc[0])*n)*mindist.y-mindist.x*n2))) {
					...
					ptres[1] = pgwd->R*((ptloc[0]+diri*(t0*rn2))*m_box.Basis+m_box.center*pgwd->scale)+pgwd->offset;
		...
		for(ivtx=0;ivtx<8;ivtx++) {
			...
			if (inrange(t0=(ptbox-ptloc[0])*diri,(float)0,dir2)) {
				...
				if (min(min(ptres[1].x-size.x,ptres[1].y-size.y),ptres[1].z-size.z)>0) {
					ptres[1] = pgwd->R*(ptres[1]*m_box.Basis+m_box.center*pgwd->scale)+pgwd->offset;
					return 1;
				}
			}
		}
...
void CBoxGeom::DrawWireframe(IPhysRenderer *pRenderer, geom_world_data *gwd, int iLevel, int idxColor)
{
	...
	/*int i,j;
	Vec3 pt[8], size=m_box.size*pgwd->scale, offset=pgwd->R*m_box.center*pgwd->scale+pgwd->offset;
	Matrix33 R; 
	if (!m_box.bOriented) R = pgwd->R;
	else R = pgwd->R*m_box.Basis.T();
	for(i=0;i<8;i++)
