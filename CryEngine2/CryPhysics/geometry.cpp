...
char g_BoxIdBuf[3];
surface_desc g_BoxSurfaceBuf[3];
edge_desc g_BoxEdgeBuf[3];

// cylindergeom-related
vector2df g_CylCont[64];
int g_CylContId[64];
char g_CylIdBuf[1];
surface_desc g_CylSurfaceBuf[1];
edge_desc g_CylEdgeBuf[1];

BBox g_BBoxBuf[128];
int g_BBoxBufPos;

// heightfieldbv-related
BVheightfield g_BVhf;
...
int CGeometry::Intersect(IGeometry *pCollider, geom_world_data *pdata1,geom_world_data *pdata2, intersection_params *pparams, geom_contact *&pcontacts)
{
	...
	iCaller=gtest[0].iCaller=gtest[1].iCaller = iszero((int)GetCurrentThreadId()-g_physThreadId)^1;
	...
	WriteLockCond lock(g_lockIntersect,pparams->bThreadSafe^1);
	pparams->plock = &g_lockIntersect;
...
float CGeometry::BuildOcclusionCubemap(geom_world_data *pgwd, int iMode, int *pGrid0[6],int *pGrid1[6],int nRes, float rmin,float rmax, int nGrow)
{
	...
	int i,j,irmin;
	rcd.iCaller = iszero((int)GetCurrentThreadId()-g_physThreadId)^1;
													...
													intersection_params *pparams, geom_contact *&pcontacts)
{
	...
	if (max(g_BrdPtBufPos+8-(int)(sizeof(g_BrdPtBuf)/sizeof(g_BrdPtBuf[0])), g_nTotContacts-(int)(sizeof(g_Contacts)/sizeof(g_Contacts[0])))>=0)
		return 0;
	pcontacts = g_Contacts+g_nTotContacts;
	...
	if (!G(Overlapper).Check(pBV1->type,pBV2->type, *pBV1,*pBV2))
		return 0;
	...
	if (!pparams->bSweepTest) {
		...
		} else if (!G(Overlapper).Check(itype[0],itype[1], pprim[0],pprim[1]))
			return 0;
