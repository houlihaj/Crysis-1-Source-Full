...
*/
  if (!m_pDummyNode)
	{
		m_dummyNodeIndex = GetAISystem()->GetNodeGraph()->CreateNewNode(IAISystem::NAV_FREE_2D, ZERO);
    m_pDummyNode = GetAISystem()->GetNodeGraph()->GetNode(m_dummyNodeIndex);
	}
...
void CFree2DNavRegion::Clear()
{
  if (m_pDummyNode)
  {
    GetAISystem()->GetNodeGraph()->Disconnect(m_dummyNodeIndex);
    m_pDummyNode = 0;
		m_dummyNodeIndex = 0;
  }
}
...
bool CFree2DNavRegion::CheckPassability(const Vec3& from, const Vec3& to, 
                                        float radius, const NavigationBlockers& navigationBlockers) const
{
  const SpecialArea *sa = GetAISystem()->GetSpecialArea(from, SpecialArea::TYPE_FREE_2D);
  if (!sa)
    return false;
...
bool CFree2DNavRegion::GetSingleNodePath(const GraphNode *pNode, 
                                         ...
                                         const NavigationBlockers &navigationBlockers, 
                                         std::vector<PathPointDescriptor> &points) const
{
  const SpecialArea *sa = GetAISystem()->GetSpecialAreaNearestPos(startPos, SpecialArea::TYPE_FREE_2D);
  if (!sa)
    return false;
