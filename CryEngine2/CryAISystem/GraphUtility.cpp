...
int CGraph::ProcessMegaMerge(const string & areaName, ListNodeIds& nodesList, const Vec3r& vCutStart, const Vec3r& vCutEnd )
{
  ...
  int	cutEnd = GetAISystem()->m_VertexList.FindVertex( tmpObst );
  tmpObst.vPos = vCutStart;
  int	cutStart = GetAISystem()->m_VertexList.FindVertex( tmpObst );
  ...
  for(it=nodesList.begin(); it!=nodesList.end(); it++)
  {
    ...
    if( newLink )
    {
      ...
      unsigned nbrLink;
      real curCross = curNode->GetTriangularNavData()->GetCross( GetLinkManager(), vCutStart, cutN, newLink );
      ...
      real nbrCross = nbrNode->GetTriangularNavData()->GetCross( GetLinkManager(), vCutStart, cutN, (nbrLink) );
  ...
  for(it=nodesList.begin(); it!=nodesList.end(); it++)
  {
    ...
    Vec3r theOtherVertex = GetAISystem()->m_VertexList.GetVertex(curNode->GetTriangularNavData()->vertices[otherVertexIdx1]).vPos
      - vCutStart;
    theOtherVertex.normalize();
    real curCross = cutN.x * theOtherVertex.y - cutN.y * theOtherVertex.x;
    theOtherVertex = GetAISystem()->m_VertexList.GetVertex(curNode->GetTriangularNavData()->vertices[otherVertexIdx2]).vPos
      - vCutStart;
    theOtherVertex.normalize();
...
void CGraph::TriangulateOutline(const string & areaName, ListNodeIds& nodesList, ObstacleIndexList&	outline, bool orientation )
{
  ...
  while(outline.size()>3)
  {
    ...
    for(ObstacleIndexList::iterator curItr=outline.begin(); curItr!=outline.end(); curItr++,cntr++)
    {
      ...
      notGood = false;
      if(tmpNode.GetTriangularNavData()->IsAntiClockwise()!=orientation)
        notGood = true;
      tmpNode.GetTriangularNavData()->MakeAntiClockwise();
      ...
      //				else if(PointInTriangle(*tmpItr, &tmpNode))
        else if(PointInTriangle(GetAISystem()->m_VertexList.GetVertex(*tmpItr).vPos, &tmpNode))
          notGood = true;

      if( notGood )	// this triangle can't be used
        continue;

      real	dVal = tmpNode.GetTriangularNavData()->GetDegeneracyValue();
    ...
    candidateNode->GetTriangularNavData()->MakeAntiClockwise();
