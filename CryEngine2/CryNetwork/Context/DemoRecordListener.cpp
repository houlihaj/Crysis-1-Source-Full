...
CDemoRecordListener::CDemoRecorderChannel::CDemoRecorderChannel(CDemoRecordListener* pParent) : m_connected(true), m_pParent(pParent)
{
	...
	string connectionString = pParent->m_pContext->GetGameContext()->GetConnectionString(true);
...
static std::auto_ptr<CSimpleOutputStream> CreateOutput( const char * filename )
{
	std::auto_ptr<CSimpleOutputStream> out;
	std::auto_ptr<CCompressingOutputStream> stream;
	stream.reset( new CCompressingOutputStream() );
	...
	m_pContext(pContext), m_output(CreateOutput(filename)), m_channel(this)
{
	...
	char version[32]; gEnv->pSystem->GetProductVersion().ToString(version);
...
void CDemoRecordListener::DoUpdateObject(const SContextObjectRef& obj, uint8 aspects)
{
	SCOPED_GLOBAL_LOCK;
...
void CDemoRecordListener::DoSetAspectProfile(SNetObjectID id, uint8 aspects, uint8 profile)
{
	SCOPED_GLOBAL_LOCK;
