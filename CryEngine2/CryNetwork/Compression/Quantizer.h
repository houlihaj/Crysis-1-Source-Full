...
private:
	...
	ILINE void QuantizeRLM(uint32 &quantizedValue, double fScaled) const
	{
		...
		if (fScaled < 0.5)
			...
			quantizedValue = uint32( (fScaled-0.5) * (m_bitsAsDoubles[m_nBits-1]-1) + m_bitsAsDoubles[m_nBits-1] + 0.5 );
	}
	...
	{
		dequantizedValue = nValue * m_fRange / m_bitsAsDoubles[m_nBits] + m_fMin;
	}

	ILINE void DequantizeTC(float &dequantizedValue, uint32 nValue) const
	{
		dequantizedValue = (nValue + 0.5f ) * m_fRange / m_bitsAsDoubles[m_nBits] + m_fMin;	
	}

	ILINE void DequantizeRL(float &dequantizedValue, uint32 nValue) const
	{
		dequantizedValue = nValue * m_fRange / (m_bitsAsDoubles[m_nBits]-1) + m_fMin;
	}
	...
	ILINE void DequantizeRLM(float &dequantizedValue, uint32 nValue) const
	{
		...
		if (nValue < (1u<<(m_nBits-1)))
			fScaled = nValue / m_bitsAsDoubles[m_nBits-1];
		else
			fScaled = 0.5f + (nValue - (1u<<(m_nBits-1))) / m_bitsAsDoubles[m_nBits-1];
