...
/*	
	...
	*/
	...
	virtual bool	CheckInside(const Vec3 &pos,bool bWorldSpace) { return(false); }
	...
	Vec3	GetBBoxMin(bool bWorldSpace=false)
	{ 	
		if (bWorldSpace)
		{
			Vec3 vWorldPos=m_vOrigin+m_vMins;
			return(vWorldPos);
		}
	...
	Vec3	GetBBoxMax(bool bWorldSpace=false) 
	{ 
		if (bWorldSpace)
		{
			Vec3 vWorldPos=m_vOrigin+m_vMaxs;
			return(vWorldPos);
		}
		return(m_vMaxs); 
	}
