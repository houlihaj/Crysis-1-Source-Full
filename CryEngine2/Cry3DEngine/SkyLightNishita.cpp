...
const f64 c_avgDensityHeightMie( 1.2 );
const f64 c_avgDensityHeightMieInv( 1.0 / 1.2 );
const f64 c_avgDensityHeightRayleigh( 7.994 );
...
const uint32 cOLUT_HeightSteps( 32 );
const uint32 cOLUT_AngularSteps( 256 );
const uint32 cPLUT_AngularSteps( 256 );
...
static inline f64 MapSaveExpArg( double arg )
{
	const double c_saveExpArgRange( 650.0 ); // -650.0 to 650 range is safe not to introduce fp over-/underflows
...
#	endif
	...
	return( e.d );
#elif defined(_CPU_G5) // for big endian (tested on Xenon)	
	eco e;
#	ifdef _DEBUG
		e.d = 1.0;
...
#	endif
	...
	return( e.d );
#else // fall back to default exp() implementation for untested/unsupported target platforms
...
static inline f64 IntegrateOpticalDepthInternal( const Vec3_f64& start, const f64& startScale, 
	const Vec3_f64& end, const f64& endScale, const f64& avgDensityHeightInv, const f64& error )
{
	assert( _finite( startScale ) &&  _finite( endScale ) );

	Vec3_f64 mid( 0.5 * ( start + end ) );
	...
	{
		// refine section via recusing down left and right branch
...
void CSkyLightNishita::SetAtmosphericConditions( const Vec3& sunIntensity, 
...
void CSkyLightNishita::SetRGBWaveLengths( const Vec3& rgbWaveLengths )
{
...
void CSkyLightNishita::SetSunDirection( const Vec3& sunDir )
{
...
CSkyLightNishita::SOpticalDepthLUTEntry CSkyLightNishita::LookupBilerpedOpticalDepthLUTEntry( uint32 heightIndex, const f32& cosVertAngle ) const
{
	uint32 vertAngleIndex; f32 vertAngleIndexFrc;
	MapCosVertAngleToIndex( cosVertAngle, vertAngleIndex, vertAngleIndexFrc );

	if( vertAngleIndex >= cOLUT_AngularSteps - 1 )
	{
		return( m_opticalDepthLUT[ OpticalLUTIndex( heightIndex, vertAngleIndex ) ] );
	}
	else
	{
		uint32 index( OpticalLUTIndex( heightIndex, vertAngleIndex ) );
		const SOpticalDepthLUTEntry& a( m_opticalDepthLUT[ index ] );
		const SOpticalDepthLUTEntry& b( m_opticalDepthLUT[ index + 1 ] );
...
const CSkyLightNishita::SOpticalScaleLUTEntry& CSkyLightNishita::LookupOpticalScaleLUTEntry( uint32 heightIndex ) const
{
	assert( heightIndex < cOLUT_HeightSteps );
	return( m_opticalScaleLUT[ heightIndex ] );
}
	...
	Vec3& partialInScatteringMie, Vec3& partialInScatteringRayleigh ) const
{
	...
	Vec3 sampleRes( exp_fast( sampleExpArg.x ), exp_fast( sampleExpArg.y ), exp_fast( sampleExpArg.z ) );
	...
	Vec3& inScatteringMieNoPremul, Vec3& inScatteringRayleighNoPremul ) const
{
	...
	SOpticalDepthLUTEntry odAtViewerSky( LookupBilerpedOpticalDepthLUTEntry( 0, skyDir.Dot( c_up ) ) );
	SOpticalDepthLUTEntry odAtViewerSun( LookupBilerpedOpticalDepthLUTEntry( 0, m_sunDir.Dot( c_up ) ) );

	// sample partial in-scattering term at viewer
	Vec3 curSampleMie, curSampleRayleigh;
	SamplePartialInScatteringAtHeight( LookupOpticalScaleLUTEntry( 0 ), outScatteringConstMie, outScatteringConstRayleigh, 
	...
	for( int a( 1 ); a < cOLUT_HeightSteps; a += m_inScatteringStepSize )
	{	
		// calculate intersection with current "atmosphere shell"
		SOpticalScaleLUTEntry osAtHeight( LookupOpticalScaleLUTEntry( a ) );
		...
		SOpticalDepthLUTEntry odAtHeightSky( LookupBilerpedOpticalDepthLUTEntry( a, skyDir.Dot( newUp ) ) );
		SOpticalDepthLUTEntry odAtHeightSun( LookupBilerpedOpticalDepthLUTEntry( a, m_sunDir.Dot( newUp ) ) );
	...
	Vec3* pInScatteringRayleighNoPremul, Vec3* pInScatteringRayleigh  ) const
{
	...
	Vec3 outScatteringConstRayleigh( 4.0f * c_pif * m_Kr * m_invRGBWaveLength4 );
	Vec3 inScatteringConstRayleigh( ( m_sunIntensity * m_Kr * phaseForPhiGRayleigh ).CompMul( m_invRGBWaveLength4 ) );

	// compute in-scattering
	Vec3 inScatteringMieNoPremul; Vec3 inScatteringRayleighNoPremul;	
...
f64 CSkyLightNishita::IntegrateOpticalDepth( const Vec3_f64& start, const Vec3_f64& end, const f64& avgDensityHeightInv, const f64& error ) const
{
...
bool CSkyLightNishita::ComputeOpticalDepth( const Vec3_f64& cameraLookDir, const f64& cameraHeight, const f64& avgDensityHeightInv, float& depth ) const
{
	// init camera position
	Vec3_f64 cameraPos( 0.0, cameraHeight + c_earthRadius, 0.0 );
	...
	if( false != hitsEarth )
	{
		depth = c_opticalDepthWhenHittingEarth;
		return( false );
	}
	...
	Vec3_f64 lastCameraPos( cameraPos );
	for( int i( 1 ); i < numInitialSamples; ++i )
	{
		Vec3_f64 curCameraPos( cameraPos + cameraLookDir * ( t * ( (float) i / (float) numInitialSamples ) ) );        
...
void CSkyLightNishita::ComputeOpticalLUTs()
{
	...
	for( int a( 0 ); a < cOLUT_HeightSteps; ++a )
	{
		...
		for( int i( 0 ); i < cOLUT_AngularSteps; ++i )
		{
			...
			Vec3_f64 cameraLookDir( sqrt( 1.0 - cosVertAngle * cosVertAngle ), cosVertAngle, 0.0 );
			...
			if( false == b0 && false == b1 && i > 0 ) 
			{
				...
				e.mie = (f32) ( 0.5 * ( e.mie + c_opticalDepthWhenHittingEarth ) );
				e.rayleigh = (f32) ( 0.5 * ( e.rayleigh + c_opticalDepthWhenHittingEarth ) );
			}
...
f64 CSkyLightNishita::MapIndexToHeight( uint32 index ) const
{
	...
	f64 x( (f64) index / (f64) ( cOLUT_HeightSteps - 1 ) );
...
f64 CSkyLightNishita::MapIndexToCosVertAngle( uint32 index ) const
{
	...
	return( 1.0 - 2.0 * (f64) index / (f64) ( cOLUT_AngularSteps - 1 ) );
}
...
void CSkyLightNishita::MapCosVertAngleToIndex( const f32& cosVertAngle, uint32& index, f32& indexFrc ) const
{
	//assert( -1 <= cosVertAngle && 1 >= cosVertAngle );
