...
IReadStream_AutoPtr CRefStreamEngine::StartRead (const char* szSource, const char* szFilePathPC, IStreamCallback* pCallback, StreamReadParams* pParams)
{
	...
	FILE * pFile = m_pPak->FOpen(szFilePath, "rb");
	if(!pFile)
		m_pLog->LogError("StreamEngine: StartRead failed to open file %s\n",szFilePath);
	else
	{
		m_pPak->FSeek(pFile,pParams->nOffset,SEEK_SET);
		if(!m_pPak->FRead((unsigned char*)pParams->pBuffer, pParams->nSize, pFile))
			m_pLog->LogError("StreamEngine: StartRead failed to read file %s\n",szFilePath);
		GetPak()->FClose(pFile);
	}
	pProxy->FinalizeIO();
//	AddIOJob (pProxy);
