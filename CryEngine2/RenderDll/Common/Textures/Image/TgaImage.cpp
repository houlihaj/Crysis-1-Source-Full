...
CImageTgaFile::CImageTgaFile (byte* ptr, long filesize) : CImageFile ()
{
  ...
  mfSet_Flags(FIM_NOTSUPPORTS_MIPS);
  mfSet_dimensions (cols, rows);
  mfSet_ImageSize(cols * rows * 4);
  pixels = (SRGBPixel *)mfGet_image(0);
  ...
  SwapEndian((int32*)pixels, cols * rows);
}
...
static void bwrite(unsigned char data)
{
  ...
  iSystem->GetIPak()->FWrite(d, 1, 1, sFileData);
}
...
bool WriteTGA8(byte *data8, int width, int height, char *filename)
{
  ...
  bool bRet = WriteTGA(data32, width, height, filename,32,32);
  delete [] data32;
...
bool WriteTGA(byte *data, int width, int height, const char *filename, int src_bits_per_pixel, int dest_bits_per_pixel)
{
  ...
  unsigned long r,g,b,a;
	...
	// filename needs to be relative !!

	// construct path with current folder - otherwise path would start at mastercd/game 
	char szCurrDir[ICryPak::g_nMaxPath];
	CryGetCurrentDirectory(ICryPak::g_nMaxPath,szCurrDir);

	string fullPath = string(szCurrDir)+"/"+filename;

  if ((sFileData = iSystem->GetIPak()->FOpen(fullPath.c_str(), "wb")) == NULL)
    return false;
...
/*
  ...
  if( src_bytes_per_pixel == 4 )
	{
		// Set alpha to 1 to make output work with apps that premultiply color with alpha
		byte *pData = data;
		for (i = 0; i < hxw; i++)
		{
			*(pData+3) = 255;
			pData += src_bytes_per_pixel;
		}
	}
  ...
  {	
    ...
    memcpy(d, data, hxw*4);
    SwapEndian((uint *)d, hxw);
    iSystem->GetIPak()->FWrite(d, hxw, src_bytes_per_pixel, sFileData);
    SAFE_DELETE_ARRAY(d);
#else
    iSystem->GetIPak()->FWrite(data, hxw, src_bytes_per_pixel, sFileData);
#endif
  }
  else
  {
  ...
  iSystem->GetIPak()->FClose(sFileData);
