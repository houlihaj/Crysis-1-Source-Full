...
decode_mcu (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    /* Output the DC coefficient (assumes jpeg_nat_order[0] = 0) */
    (*block)[0] = (JCOEF) s;
	...
	if (s) {
	   ...
	   * Note: the extra entries in jpeg_nat_order[] will save us
	   * if k >= DCTSIZE2, which could happen if the data is corrupted.
	   */
	  (*block)[jpeg_nat_order[k]] = (JCOEF) s;
	} else {
	  if (r != 15)
	    break;
	  k += 15;
...
GLOBAL void
jinit_huff_decoder_jpeg (j_decompress_ptr cinfo)
{
