...
HRESULT WINAPI DXUTFindValidDeviceSettings( DXUTDeviceSettings* pOut, DXUTDeviceSettings* pIn, DXUTMatchOptions* pMatchOptions )
{
    ...
    if( bAppSupportsD3D10 && !bPreferD3D9 )
    {
#ifdef DIRECT3D10
...
#if defined (DIRECT3D9) || defined (OPENGL)
    if( bAppSupportsD3D9 && !bFoundValidD3D10 )
    {
        // Force an enumeration with the IsDeviceAcceptable callback
...
//--------------------------------------------------------------------------------------
CD3D10Enumeration* WINAPI DXUTGetD3D10Enumeration( bool bForceEnumerate )
{
    // Using an accessor function gives control of the construction order
    static CD3D10Enumeration d3denum;
...
void CD3D10Enumeration::GetResolutionMinMax(UINT& nMinWidth, UINT& nMinHeight, 
                                           UINT& nMaxWidth, UINT& nMaxHeight )
{
  nMinWidth = m_nMinWidth;
  nMinHeight = m_nMinHeight;
                                      ...
                                      void* pIsD3D10DeviceAcceptableFuncUserContext )
{
    ...
    for( int i = 0; i < m_AdapterInfoList.GetSize(); i++ )
    {
        ...
        StringCchCopyW( pAdapterInfo->szUniqueDescription, 100, pAdapterInfo->AdapterDesc.Description );
        if( !bUniqueDesc )
        {
            WCHAR sz[100];
            StringCchPrintfW( sz, 100, L" (#%d)", pAdapterInfo->AdapterOrdinal );
            StringCchCatW( pAdapterInfo->szUniqueDescription, DXGI_MAX_DEVICE_IDENTIFIER_STRING, sz );
        }
    }
...
HRESULT CD3D10Enumeration::EnumerateDevices( CD3D10EnumAdapterInfo *pAdapterInfo )
{
  ...
  for( UINT iDeviceType = 0; iDeviceType < devTypeArrayCount; iDeviceType++ )
  {
...
#if defined(PS3)
		//we do not need this initializationtests as we're sure about our CryDXPSDevice aviability
		hr =	(devTypeArray[iDeviceType] == D3D10_DRIVER_TYPE_HARDWARE)-1;
#else
    ...
    BuildMultiSampleQualityList( fmt, pd3dDevice, pDeviceInfo );
...
//--------------------------------------------------------------------------------------
void CD3D10Enumeration::BuildMultiSampleQualityList( DXGI_FORMAT fmt, ID3D10Device *pd3dDevice, CD3D10EnumDeviceInfo* pDeviceInfo )
{
  for( int i = 1; i <= D3D10_MAX_MULTISAMPLE_SAMPLE_COUNT ; ++i )
  {
    ...
    if( SUCCEEDED( pd3dDevice->CheckMultisampleQualityLevels( fmt, i, &Quality ) ) &&
      Quality > 0 )
    {
      pDeviceInfo->multiSampleCountList.Add( i );
      if( Quality > m_nMultisampleQualityMax + 1 )
        Quality = m_nMultisampleQualityMax + 1;
      pDeviceInfo->multiSampleQualityList.Add( Quality );
    }
  }
}
...
UINT WINAPI DXUTGetDXGIColorBits( DXGI_FORMAT fmt )
{
  switch( fmt )
  {
  case DXGI_FORMAT_R32G32B32A32_TYPELESS:
                                          ...
                                          DXUTMatchOptions* pMatchOptions )
{
    ...
    // Create flags
    //---------------------
    pOptimalDeviceSettings->CreateFlags = 0;
                                                   ...
                                                   DXUTMatchOptions* pMatchOptions )
{
    ...
    {
        bool bFound = false;
        for( int i=0; i<pDeviceSettingsCombo->pDeviceInfo->multiSampleCountList.GetSize(); i++ )
        {
            UINT Count = pDeviceSettingsCombo->pDeviceInfo->multiSampleCountList.GetAt(i);
            UINT Quality = pDeviceSettingsCombo->pDeviceInfo->multiSampleQualityList.GetAt(i);
                                ...
                                DXGI_MODE_DESC* pAdapterDisplayMode )
{
    ...
    if( pDeviceSettingsCombo->DeviceType == pOptimalDeviceSettings->DriverType )
        fCurRanking += fDeviceTypeWeight;
    // Slightly prefer HAL 
		//refactore me: correct the comparison of different enums
    if( (int)pDeviceSettingsCombo->DeviceType == (int)D3DDEVTYPE_HAL )
        fCurRanking += 0.1f; 
    ...
    {
        int nBitDepthDelta = abs( (int)((long) DXUTGetDXGIColorChannelBits(pDeviceSettingsCombo->BackBufferFormat) -
...
{
  DXGI_MODE_DESC adapterDisplayMode;
  DXUTGetD3D10AdapterDisplayMode( pBestDeviceSettingsCombo->AdapterOrdinal, 0, &adapterDisplayMode );
  ...
  else
  {
    ...
    else if( pMatchOptions->eMultiSample == DXUTMT_CLOSEST_TO_INPUT )   
    {
      ...
      for( int i=0; i < pBestDeviceSettingsCombo->pDeviceInfo->multiSampleCountList.GetSize(); i++ )
      {
        UINT Count = pBestDeviceSettingsCombo->pDeviceInfo->multiSampleCountList.GetAt(i);
        UINT Quality = pBestDeviceSettingsCombo->pDeviceInfo->multiSampleQualityList.GetAt(i);
...
#ifdef PS3
	//get rid of warning
	HWND ouputWin = (HWND)((pBestDeviceSettingsCombo->Windowed)? DXUTGetHWNDDeviceWindowed() : DXUTGetHWNDDeviceFullScreen());
  pValidDeviceSettings->sd.OutputWindow                = (int)ouputWin;
#else
...
#if (defined(DEBUG) || defined(_DEBUG)  ) && !defined(PS3)
  // Default to debug device when doing a debug build
  // The app can override this in ModifyDeviceSettings
  //pValidDeviceSettings->CreateFlags                    = D3D10_CREATE_DEVICE_DEBUG;
#endif
  pValidDeviceSettings->CreateFlags                   |= D3D10_CREATE_DEVICE_SINGLETHREADED;
}
                                      ...
                                      DXGI_MODE_DESC displayModeIn, DXGI_MODE_DESC* pBestDisplayMode )
{
    ...
    {
        // In windowed mode, all resolutions are valid but restritions still apply 
        // on the size of the window.  See DXUTChangeD3D10Device() for details
        *pBestDisplayMode = displayModeIn;
    }
    else
    {
...
HRESULT WINAPI DXUTGetD3D10AdapterDisplayMode( UINT AdapterOrdinal, UINT nOutput, DXGI_MODE_DESC *pModeDesc )
{
  ...
  if( pOutputInfo )
  {
    ...
    pModeDesc->Height = Desc.DesktopCoordinates.top - Desc.DesktopCoordinates.bottom;
  }
