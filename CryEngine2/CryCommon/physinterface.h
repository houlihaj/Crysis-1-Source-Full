...
struct pe_params_car : pe_params {
	...
	pe_params_car() {
		...
		MARK_UNUSED slipThreshold,gearDirSwitchRPM,kDynFriction,minBrakingFriction,maxBrakingFriction,steerTrackNeutralTurn,maxGear,minGear,pullTilt, noShootImpulse;
	}
  ...
  float pullTilt; // tilt angle of pulling force towards ground [rad]	
	int noShootImpulse; //big hack for expansion pack cargo level. AI driven vehicle turn over fix
};
...
struct PhysicsVars : SolverSettings {
  int bFlyMode;
	...
	float netAngSnapMul;
	float netSmoothTime;
	int		bEnableTriangleCache;
	float	fSleepSpeedMultiple;
};
