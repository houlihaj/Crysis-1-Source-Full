...
inline float half2float(uint16 half)
{
	if (half & 0x7c00)
	{
		uint32 tmp = ((half & 0x8000) << 16) + ((half & 0x7fff) << 13) + 0x38000000;
		return (float &)tmp;
	}
	else
	{
		return (half & 0x7fff) * (half & 0x8000 ? -1.0f / 65536.0f / 256.0f : 1.0f / 65536.0f / 256.0f);	// Denormalized number
		// without conditional jump
		//float tmp = (half & 0x7fff) * (1.0f / 65536.0f / 256.0f);
		//uint32 tmp2 = (half << 16 & 0x80000000) + (uint32&)(tmp);
		//return (float&)tmp2;
	}
}

inline float half2float_safe(uint16 half)
{
	if (half & 0x7c00)
	{
		uint32 tmp;
		if (0x7c00 == (half & 0x7c00))
		{
			if (half & 0x0200)
				tmp = ((half & 0x8000) << 16) + 0x7fc00000;	// QNaN
			else if (half & 0x01ff)
				tmp = ((half & 0x8000) << 16) + 0x7f800001;	// SNaN
			else
				tmp = ((half & 0x8000) << 16) + 0x7f800000;	// Inf
			return (float &)tmp;
		}
		tmp = ((half & 0x8000) << 16) + ((half & 0x7fff) << 13) + 0x38000000;
		return (float &)tmp;
	}
	else
		return (half & 0x7fff) * (half & 0x8000 ? -1.0f / 65536.0f / 256.0f : 1.0f / 65536.0f / 256.0f);
}
