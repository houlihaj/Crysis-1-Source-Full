...
enum eVertexFormat
{
  ...
  VERTEX_FORMAT_COL4UB_COL4UB=15,      // SH coefficients
	...
	VERTEX_FORMAT_SCATTER=17,						 // per vertex scattering information
...
#endif
{
...
#ifdef FP16_MESH
		assert(i <= 3);
		return XMConvertHalfToFloat(((HALF *)this)[i]);
#else
		assert(i <= 2);
		return ((f32 *)this)[i];
#endif
	...
	_inline Vec3f16& operator = (const Vec3& sl)
	{
		...
		__stvewx(ZW, &x, 4);
#else
		x = XMConvertFloatToHalf(sl.x);
		y = XMConvertFloatToHalf(sl.y);
		z = XMConvertFloatToHalf(sl.z);
		w = XMConvertFloatToHalf(1.0f);
#endif
#endif
		return *this;
	...
	_inline Vec3 ToVec3() const
	{
		...
		v.z = z;
#else
		v.x = XMConvertHalfToFloat(x);
		v.y = XMConvertHalfToFloat(y);
		v.z = XMConvertHalfToFloat(z);
#endif
		return v;
	}
};
...
struct struct_VERTEX_FORMAT_S1F
{
	float scatter;
};
  ...
  switch(nFormat)
  {
    ...
    case VERTEX_FORMAT_P3F_COL4UB_RES4UB_TEX2F_PS4F:
      ...
      return new struct_VERTEX_FORMAT_P3F_COL4UB_RES4UB_TEX2F_PS4F[nVerts];
		...
		case VERTEX_FORMAT_SCATTER:
			return new struct_VERTEX_FORMAT_S1F[nVerts];
...
enum EStreamIDs
{
  ...
  VSF_SH_INFO,									// SH buffer
	...
	VSF_SCATTER,									// Per vertex scattering information for low quality render
