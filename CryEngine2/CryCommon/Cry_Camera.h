...
public:
  ...
  bool IsAABBVisible_FM( const AABB& aabb ) const;
	...
	CCamera()	{	m_Matrix.SetIdentity();	SetFrustum(640,480); m_zrangeMin = 0.0f; m_zrangeMax = 1.0f;	m_pMultiCamera = NULL; m_pPortal = NULL; m_JustActivated = false;}
	~CCamera() {}

	void GetFrustumVertices(Vec3 * pVerts) const;
	void GetFrustumVerticesCam(Vec3 * pVerts) const;

	void SetJustActivated(const bool justActivated) {m_JustActivated = justActivated;}
	bool IsJustActivated() const {return m_JustActivated;}
...
private:		
  ...
  class PodArray<CCamera> * m_pMultiCamera; // maybe used for culling instead of this camera
	...
	bool m_JustActivated;		//Camera activated in this frame, used for disabling motion blur effect at camera changes in movies
};
