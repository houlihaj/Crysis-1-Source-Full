...
struct IVariable : public CRefCountBase
{
	...
	*/
	...
	enum EDataType
	{
		...
		DT_CURVE = 64,	// Combined with other types
	};
	...
	enum EFlags
	{
		...
		UI_DISABLED					= 0x01,		//!< This variable will be disabled in UI.
		UI_BOLD							= 0x02,		//!< Variable name in properties will be bold.
		UI_SHOW_CHILDREN		= 0x04,		//!< Display children in parent field.
		UI_USE_GLOBAL_ENUMS = 0x08,   //!< Use CUIEnumsDatabase to fetch enums for this variable name.
	};
  ...
  virtual bool DeleteChildVar( IVariable* var, bool recursive = false ) = 0;
	...
	//////////////////////////////////////////////////////////////////////////
	//! Set value limits.
	virtual void SetLimits( float fMin, float fMax, bool bHardMin = true, bool bHardMax = true ) = 0;
	//! Get value limits. Return true if hard limit.
	virtual void GetLimits( float& fMin, float& fMax, bool& bHardMin, bool& bHardMax ) = 0;
	void GetLimits( float& fMin, float& fMax )
	{
		bool b;
		GetLimits( fMin, fMax, b, b );
	}
...
namespace var_type
{
	...
	//! Put Vec3 to out stream operator.
	inline std::ostream&	operator<<( std::ostream &stream, const Ang3& v )
	{
		stream << v.x << "," << v.y << "," << v.z;
		return stream;
	}
...
//	template <class From,class To>
	struct type_convertor
	{
		...
		void operator()( const Vec3& value,CString &to ) const { to.Format("%g,%g,%g",value.x,value.y,value.z); };
		void operator()( const Ang3& value,CString &to ) const { to.Format("%g,%g,%g",value.x,value.y,value.z); };
		void operator()( const Quat& value,CString &to ) const { to.Format("%g,%g,%g,%g",value.w,value.v.x,value.v.y,value.v.z); };
		...
		void operator()( const CString &from,Vec3& value ) const
		{
			value.Set(0,0,0);
			sscanf( (const char*)from,"%f,%f,%f",&value.x,&value.y,&value.z ); 
		};
		void operator()( const CString &from,Ang3& value ) const
		{
			value.Set(0,0,0);
			sscanf( (const char*)from,"%f,%f,%f",&value.x,&value.y,&value.z ); 
		};
		void operator()( const CString &from,Quat& value ) const
		{
			value.SetIdentity();
...
public:
	...
	void AddVariable( CVariableArray &table,CVariableBase &var,const CString &varName,VarOnSetCallback cb=NULL,unsigned char dataType=IVariable::DT_SIMPLE );
