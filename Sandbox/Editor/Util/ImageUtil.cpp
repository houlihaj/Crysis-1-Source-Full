...
bool CImageUtil::SaveBitmap( const CString &szFileName, CImage &inImage,bool inverseY )
{
	...
	uchar *pImage = new uchar[(dwWidth*3+iAddToPitch)*dwHeight];
...
//===========================================================================
bool CImageUtil::LoadBmp( const CString &fileName,CImage &image, const RECT & rc )
{
#pragma pack(push,1)
	struct SRGBcolor
	{
...
//////////////////////////////////////////////////////////////////////////
bool CImageUtil::LoadImage( const CString &fileName, CImage &image, bool *pQualityLoss )
{
  ...
  char ext[_MAX_EXT];
	...
	{
		CImageTIF tif;
		return tif.Load( fileName,image );
	} 
	...
	else if (stricmp(ext,".dds") == 0)
	{
		// Load DDS file.
		CImage_DXTC dds;
		return dds.Load( fileName,image,pQualityLoss );
	}
...
//////////////////////////////////////////////////////////////////////////
void CImageUtil::DownScaleSquareTextureTwice( const CImage &srcImage,CImage &trgImage )
{
	uint* pSrcData = srcImage.GetData();
	...
	uint* pDstData = trgImage.GetData();
	...
	{ 
		...
		for (int i=0; i<3; i++)
		{
			for (int j=0; j<3; j++)
			{
				uint col = pSrcData[((y + nSrcHeight + i - 1) % nSrcHeight) * nSrcWidth + ((x + nSrcWidth + j - 1) % nSrcWidth)];
...
void CImageUtil::ScaleToFit( const CImage &srcImage,CImage &trgImage )
{
  ...
  unsigned int *destRow,*dest,*src,*sourceRow;
	...
	uint xratio = trgW > 0 ? (srcW << 16)/trgW : 1;
	uint yratio = trgH > 0 ? (srcH << 16)/trgH : 1;
