...
//////////////////////////////////////////////////////////////////////////
ILINE void CVegetationMap::RegisterInstance( CVegetationInstance *obj )
{
	SAFE_RELEASE_NODE( obj->pRenderNode );
	obj->pRenderNode = m_I3DEngine->GetITerrain()->AddVegetationInstance( obj->object->GetId(),obj->pos,obj->scale,obj->brightness,obj->angle );
}
...
//////////////////////////////////////////////////////////////////////////
void CVegetationMap::RepositionArea( const BBox &box, const Vec3 &offset)
{
	...
	for (int y = sy1; y <= sy2; y++)
	{
		for (int x = sx1; x <= sx2; x++)
		{
			...
			while( obj )
			{
				...
				if (obj->object && obj->pos.x >= x1 && obj->pos.x <= x2 && obj->pos.y >= y1 && obj->pos.y <= y2)
				{
					Vec3 newPos = obj->pos+offset;

					float newz = m_I3DEngine->GetTerrainElevation(newPos.x,newPos.y, obj->object->IsAffectedByVoxels());
