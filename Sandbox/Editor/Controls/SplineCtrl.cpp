...
void CSplineCtrl::UpdateToolTip()
{
	if (m_nHitKeyIndex >= 0 && m_pSpline)
	{
		...
		int cont = (m_pSpline->GetKeyFlags(m_nHitKeyIndex) & SPLINE_KEY_NONCONTINUOUS_SLOPE) ? 1 : 2;

		CString tipText;
		tipText.Format( "%.2f , %.2f [C=%d]", time*m_fTooltipScaleX, val*m_fTooltipScaleY, cont );
...
void CSplineCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	{		
		...
		case VK_SPACE :
			{
				// Cycle the spline slope.
				int flags = m_pSpline->GetKeyFlags( m_nActiveKey );
				flags ^= SPLINE_KEY_NONCONTINUOUS_SLOPE;
				m_pSpline->SetKeyFlags( m_nActiveKey, flags );
				m_pSpline->Update();
