...
void CSizingControlBar::StartTracking(UINT nHitTest, CPoint point)
{
    ...
    if (!IsSideTracking())
    {
        ...
        for (int i = 0; i < arrSCBars.GetSize(); i++)
					nMinWidth = std::max( nMinWidth, (int)(bHorz ? arrSCBars[i]->m_szMinHorz.cy : arrSCBars[i]->m_szMinVert.cx) );
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ...
    else
    {
        ...
        for (int i = nFirst; nDelta != 0 && i != nLimit; i += (bBefore ? -1 : 1))
        {
            ...
            int nDeltaT = min( nDelta,(int)((bHorz ? pBar->m_szHorz.cx : pBar->m_szVert.cy) -(bHorz ? pBar->m_szMinHorz.cx : pBar->m_szMinVert.cy)));
    ...
    for (int i = nFirst; i <= nLast; i++)
    {
        ...
        if (bIsSizingBar)
        {
            nLengthActual += bHorz ? pBar->m_szHorz.cx - 2 : 
                pBar->m_szVert.cy - 2;
            nWidthMax = max(nWidthMax,(int)(bHorz ? pBar->m_szHorz.cy : pBar->m_szVert.cx));
        }
        else
