...
{
	assert( seq != 0 );
	XmlNodeRef sequenceNode = CreateXmlNode( "Sequence" );
	seq->Serialize( sequenceNode,false, false );
	SaveXmlNode( sequenceNode,szFilePath );
}
...
void CAnimationSerializer::SaveAllSequences( const char *szPath,CPakFile &pakFile )
{
	...
	XmlNodeRef movieNode=CreateXmlNode("MovieData");
	...
	//SaveXmlNode(movieNode,sFilename.c_str());
