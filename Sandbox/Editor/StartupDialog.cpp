	...
	case 0:
		...
		if (m_cUseAsBase.GetCheck() == 0)
		{
			...
			if (nResult == IDYES)
			{
				...
				strFilePath = "EditorTemplate\\Default.hm";
				...
				*/
				...
				// Load layer settings
				strFilePath = "EditorTemplate\\Default.lay";
				if (PathFileExists(strFilePath.GetBuffer(0)))
				{
				...
				// Load detail texture
				strFilePath = "EditorTemplate\\Default.dds";
				if (PathFileExists(strFilePath.GetBuffer(0)))
				{
				...
				// Copy the JPG files
				strFilePath = "EditorTemplate\\Skybox\\12.jpg";
				if (PathFileExists(strFilePath.GetBuffer(0)))
				{
				...
				strFilePath = "EditorTemplate\\Skybox\\34.jpg";
				if (PathFileExists(strFilePath.GetBuffer(0)))
				{
					VERIFY(CopyFile(strFilePath.GetBuffer(0), (CString(szLevelRoot) + 
						"\\Skybox\\34.jpg").GetBuffer(0), FALSE));
				}
				strFilePath = "EditorTemplate\\Skybox\\5.jpg";
				if (PathFileExists(strFilePath.GetBuffer(0)))
				{
					VERIFY(CopyFile(strFilePath.GetBuffer(0), (CString(szLevelRoot) + 
...
void CStartupDialog::OnChangeLevDir() 
{
	...
	// Construct the directory name
	sprintf(szLevelRoot, "%s/Levels/%s", PathUtil::GetGameFolder(), cDialog.m_strString.GetBuffer(0));

	// Does the directory already exist ?
	if (PathFileExists(szLevelRoot))
	{
		AfxMessageBox("Directory / level already exists, choose another name !");
		return;
	}
