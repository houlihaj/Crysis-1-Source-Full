...
// -------------------------------------------------------------------------
//  File name:   TerrainObjectMan.cpp
//  Version:     v1.00
//  Created:     05/18/2005 by MichaelG
...
*/
			...
			if(!breakCond)
			{
				...
				if(RayAABBNoZ(crOrigin, crDir, crBox, minMaxAngles, cInsideBox, false))
				{
...
//		return false;
	...
	dir.x = m_RayIntersectionDir.x * cScaleXY;			dir.y = m_RayIntersectionDir.y * cScaleXY;	
...
void CTerrainObjectMan::GetMinMaxAngle(const Vec3& crOrigin, const Vec2& crHitPoint, Vec2& rMinMaxAngle, const AABB& crAABB, const bool cXAxisAdjustable) const
{
	...
	min0.Normalize();			min1.Normalize();	
	max0.Normalize();			max1.Normalize();
...
)
{
	...
	if(m_TempObjects.size() >= scMaxElems)
	{
		if(!errorOutput)
		{
			GetIEditor()->GetSystem()->GetILog()->Log("Error: More than %d objects active on map for ray casting, calculation cancelled", scMaxElems);
	...
	if((cHeight > cMinHeightForRC || radius > cMinRadius) && radius < cMaxRadius)
	{
		...
		if(NQT::NOT_ENOUGH_SUBDIV_LEVELS == cRes)//use closest object and attach it there
		{
			//do not add to quadtree but add object itself
...
{	
	...
	const uint16 cWorldFactor = m_cWidth / cGridResolution;
	...
	const uint16 cWorldMapMarksFactor = m_cWidth / cTerrainMapRes;
	...
	const uint16 cHeightWorldFactor = m_cWidth / m_ObstrMan.GetObstructionMapHeightRes();
	uint16 index = 0;
