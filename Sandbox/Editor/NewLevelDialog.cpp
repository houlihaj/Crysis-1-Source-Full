...
BOOL CNewLevelDialog::OnInitDialog()
{
	...
	if(m_bIsResize)
	{
		SetWindowText("Resize Terrain");
		CWnd * pWnd = GetDlgItem( IDC_LEVEL );
		if(pWnd)
		{
			pWnd->EnableWindow(FALSE);
			//pWnd->SetWindowText(GetIEditor()->GetEditorDocumentName());
		}
		CButton * pBtn = (CButton *) GetDlgItem( IDC_USE_TERRAIN );
		if(pBtn)
		{
