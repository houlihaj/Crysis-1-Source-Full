...
const CImage *CTerrainTexGen::GetOcclusionSurfaceTexture() const
{
...

			{
				...
				if(GetIEditor()->GetDocument()->IsNewTerranTextureSystem())
				{
					if((flags&ETTG_NOTEXTURE)==0)
					{
						...
						TerrainRGBLayer.GetSubImageStretched(fMinX,fMinY,fMinX+fInvSectorCnt,fMinY+fInvSectorCnt,sectorDiffuseImage);

						/*
						// M.M. to take a look at the result of the calculation
						char str[80];

						sprintf(str,"c:\\temp\\out%d_%d.bmp",x,y);
