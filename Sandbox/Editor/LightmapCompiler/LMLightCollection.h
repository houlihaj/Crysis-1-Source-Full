...
inline void CalcNCombineHash( const Vec3 &invValue, DWORD &inoutHash )
{
...
struct LMCompLight
{
	...
	{
		vWorldSpaceLightPos = Vec3(0.0f, 0.0f, 0.0f);
		vDirection = Vec3(1.0f, 0.0f, 0.0f);
		fRadius = 1.0f;
	...
	Vec3							vWorldSpaceLightPos;				//!<
	Vec3							vDirection;							//!< For directional / spotlights (normalized)
	float							fRadius;							//!< >=0
...
public:
	...
	void AddLight(CDLight *pLight, const CString& rName, const EntityId& rID, const bool cbCastShadow = true) 
	{
		...
		cNewLight.vDirection = Vec3(0,0,1); //pLight->m_Orientation.m_vForward;
