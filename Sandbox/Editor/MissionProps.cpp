...
void CMissionProps::OnBnClickedCreate()
{
	...
	{
		// copy from template...
		if (!CopyFile( Path::MakeFullPath("Editor\\MissionTemplate.lua"),file, FALSE))
		{
			Warning( CString("MissionScript-Template (expected in ")+Path::MakeFullPath("Editor\\MissionTemplate.lua") + ") couldn't be found !" );
			return;
		}
