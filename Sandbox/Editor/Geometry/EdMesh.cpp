...
CEdMesh::~CEdMesh()
{
	SAFE_RELEASE(m_pTempIndexedMesh);
	...
	else
	{
		if (m_bModified)
		{
			...
			if (pObj)
			{
				...
				CPakFile *pPakFile =  ar.GetGeometryPak( levelPath+"\\Geometry.pak" );
				if (pPakFile)
					SaveToCGF( m_filename,pPakFile );
			}
			SetModified(false);
		}
	}
...
CEdMesh* CEdMesh::LoadMesh( const char *filename )
{
	...
	// Make new.
	IStatObj *pGeom = GetIEditor()->Get3DEngine()->LoadStatObj( filename );
	if (!pGeom)
		return 0;
...
//////////////////////////////////////////////////////////////////////////
void CEdMesh::SetFilename( const CString &filename )
{
...
//////////////////////////////////////////////////////////////////////////
void CEdMesh::Render( SRendParams &rp )
{
...
//////////////////////////////////////////////////////////////////////////
IIndexedMesh* CEdMesh::GetIndexedMesh()
{
	if (m_pStatObj)
	{
		...
		{
			// Load from CGF.
			CString sFilename = m_pStatObj->GetFilePath();
			CContentCGF *pCGF = gEnv->p3DEngine->LoadChunkFileContent( sFilename );
			if (pCGF)
			{
				CMesh *pMesh = pCGF->GetMergedMesh();
				if (!pMesh)
				{
					for (int i = 0; i < pCGF->GetNodeCount(); i++)
					{
						if (pCGF->GetNode(i)->pMesh)
						{
							pMesh = pCGF->GetNode(i)->pMesh;
							break;
						}
					}
				}
				if (pMesh)
				{
					m_pTempIndexedMesh = GetIEditor()->Get3DEngine()->CreateIndexedMesh();
					m_pTempIndexedMesh->SetMesh( *pMesh );
					return m_pTempIndexedMesh;
				}
				gEnv->p3DEngine->ReleaseChunkFileContent(pCGF);
			}
		}
	}
	return 0;
...
void CEdMesh::UpdateIndexedMeshFromCache( bool bFast )
{
	...
	{
		//if (bFast)
			//m_pSubObjCache->pTriMesh->UpdateIndexedMeshFast( GetIndexedMesh() );
		//else
		if (bFast)
		{
...
bool CEdMesh::HitTest( HitContext &hit )
{
	...
	if (nHitTestWhat == 0)
	{
		...
		{
		case SO_ELEM_VERTEX: nHitTestWhat |= SO_HIT_ELEM_VERTEX; break;
		case SO_ELEM_EDGE:   nHitTestWhat |= SO_HIT_ELEM_EDGE; break;
		case SO_ELEM_FACE:   nHitTestWhat |= SO_HIT_ELEM_FACE; break;
		case SO_ELEM_POLYGON:nHitTestWhat |= SO_HIT_ELEM_POLYGON; break;
		}
	}
...
//////////////////////////////////////////////////////////////////////////
void CEdMesh::ModifySelection( SSubObjSelectionModifyContext &modCtx )
{
	if (!m_pSubObjCache)
		return;
...
CEdMesh* CEdMesh::CreateMesh( const char *name )
{
	...
	// Make new.
	pEdMesh->m_pStatObj = gEnv->p3DEngine->CreateStatObj();
